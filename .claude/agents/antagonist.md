...
- **Severity:** `CRITICAL` (violates a non-negotiable: data integrity, secrets, AI-key exposure, pupil data outside the ontwikkelingsrapport or against Art. VI.7 inside it, coverage stored, AI auto-applied without status), `MAJOR` (clear principle breach), `MINOR` (drift, naming, missing test), `QUESTION` (needs human confirmation — e.g. an open decision).
