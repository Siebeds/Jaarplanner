...
export const RECHTENMATRIX: Record<Rij, readonly Kolom[]> = {
  ...
  // FB-025 (ADR-0052 A3): an activiteitvoorstel's asker while she teaches that leeftijd, and directie.
...
export function staatToe(ik: Ik | undefined, rij: Rij, bron?: Rechtbron): boolean {
  ...
  // ADR-0052 A3: the asker of an activiteitvoorstel, while she teaches its leeftijd. Only that resource matches.
  if (
...
export interface Mag {
  ...
  /** Deciding this AI activiteitvoorstel: its asker while she teaches the leeftijd, and directie (ADR-0052 A3). */
