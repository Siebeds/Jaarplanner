...
export const RECHTENMATRIX: Record<Rij, readonly Kolom[]> = {
  ...
  // FB-036 (ADR-0041 W2, D3): a woordweb is its owner's; directie passes every row. Keeping one needs no row (D2).
...
export function staatToe(ik: Ik | undefined, rij: Rij, bron?: Rechtbron): boolean {
  ...
  // ADR-0041 W2: the owner of a woordweb, whatever else she holds. Only a woordweb resource matches this column.
...
export interface Mag {
  ...
  /** Changing this woordweb and asking the AI for words: its owner, and directie (ADR-0041 W2, D3). */
