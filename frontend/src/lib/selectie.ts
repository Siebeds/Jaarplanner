...
export function useActieveSelectie() {
  ...
  const { data: schooljaren, isPending: schooljarenLaden, isError: schooljarenFout } = useSchooljaren();
  ...
  const { data: alleKlassen, isPending: klassenLaden, isError: klassenFout } = useKlassen();
  ...
  return {
    ...
    /**
     * Either list failed to load. `laadt` is false then and both lists are empty, so a screen that says "there is no
     * schooljaar" or "no klas" on an empty list must ask this first: an empty list after a failure proves nothing
     * (FB-001, antagonist round 2).
     */
