 ...
 */
export function Ster({ kleur, className }: { kleur: string; className?: string }) {
  ...
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false" className={cn("h-5 w-5 shrink-0", className)}>
      <polygon
        ...
        // A style rather than presentation attributes: `var()` is honoured there in every browser.
        style={{ fill: tint?.vul ?? "none", stroke: tint?.rand ?? "currentColor" }}
        strokeWidth={1.4}
