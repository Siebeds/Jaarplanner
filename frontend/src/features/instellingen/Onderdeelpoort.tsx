...
import { ONDERDELEN, isAlleenAdmin, padVan, useZichtbareOnderdelen, type Deel } from "./onderdelen";

/**
 * The route half of hiding an admin-only part. The link is already gone for anyone else
 ...
 *
 * It renders nothing while `/api/ik` is still answering, so an admin never sees a redirect flash
...
export function Onderdeelpoort({ deel, children }: { deel: Deel; children: ReactNode }) {
  ...
  if (!onderdeel || !isAlleenAdmin(onderdeel)) return <>{children}</>;
