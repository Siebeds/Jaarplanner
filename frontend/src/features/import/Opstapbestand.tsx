...
/**
 * Loading one discipline's official Op.stap goal file in Excel (FR-2.1, re-import FR-2.5): the route that was the source
 * until KOV's API replaced it (ADR-0032). Moved out of `Opstapimport.tsx` by E1-22, unchanged in behaviour, because that
 * file is now the API flow and this upload is offered only while it can still succeed.
 *
 * **Offered only until a leerplandoelen snapshot from KOV has been applied.** From then on the server refuses every file
 * (Art. VII.2, ratified 2026-09-13), so `Opstapimport` stops rendering this component rather than ship a control that
 * can only be refused (the E3-06 rule). Before that it is the one way to load goals outside goal set G, which the API
 * import does not take.
 *
