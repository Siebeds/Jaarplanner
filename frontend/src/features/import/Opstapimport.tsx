...
export function Opstapimport() {
  ...
  /** Same rule as the school-content half: an outcome belongs to one file and one discipline, or to nothing. */
  ...
  // 409 is a refusal by the state of the loaded curriculum, not a fault in the rows, and it is the one that
  // needs its own frame. A 400 (an unknown discipline number, a missing or non-xlsx file) is the uploader's own
  // request to correct, so it takes the ordinary alert.
  ...
  return (
    ...
    >
      ...
      */}
      ...
      {/*
        The refusal panel: a system state with a named next step, kept visually and verbally apart from the row
        problems below. The server's Dutch `detail` carries the specific reason and the step ("laad eerst de
        decretale minimumdoelen in", or "controleer of dit bestand bij discipline X hoort"); the framing sentence
        above it is ours, and the fallback exists because a body can always be replaced by a proxy.
      */}
      {isWeigering ? (
        <section
          role="alert"
        ...
        >
          ...
          <p className="mt-1 text-sm leading-snug text-attentie-ink">
            {t("import.opstap.geweigerdUitleg")}
          </p>
