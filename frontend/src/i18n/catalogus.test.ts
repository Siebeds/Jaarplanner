...
describe("de Nederlandse catalogus", () => {
  ...
  it("heeft geen dode sleutels", () => {
    ...
    const viaSjabloon = /^(doelsoort|status|herkomst|activiteitsoort|activiteitkleur|telwoord)\./;
...
describe("het gebruikersbeheer (E6-04)", () => {
  it("zegt bij een leeftijd zonder hoofdleerkracht niet dat alleen de admin er iets mag, met de leeftijd in de zin zelf", () => {
    // The E5-03 rule. The sentence shows when a listed jaarfase has no appointment that counts anywhere. ADR-0030 (c)
    // says only that admin then does BY HAND what a hoofdleerkracht would. It is not "alone": themabeheer still creates
    ...
    // rounds claimed too much ("past alleen de admin de subthema's aan", then "beheert alleen de admin de subthema's
