...
- [x] **E5-01 — Coverage computation (computed, never stored)** — *done 2026-08-03 on `story/E5-01-dekkingsberekening`. One antagonist round (VIOLATIONS FOUND, 4 MAJOR + 6 MINOR + 1 QUESTION), all findings addressed; **the owner ruled that the fix round ships without a re-audit** (see below). Worklog: [`worklogs/E5-01/implementation.md`](worklogs/E5-01/implementation.md).*
  ...
  > **Still not reached: Art. V.2's inspection level.** No minimumdoel coverage is computed — that is **E5-04**. *(Read "blocked on E1-12" until 2026-09-13. Its input now exists: E1-12 loads the decreed minimumdoelen and E1-21 the G goals concorded to them, and by owner ruling of that day E5-04 is what makes minimumdoel-level coverage real.)* Each doel carries its `minimumdoelRef` so the roll-up needs no second pass, and a named test exists so the field's presence is not read as the roll-up's presence.
...
- [x] **E5-03 — Coverage % + missing-goals list + doelsoort filter** — *done 2026-08-06*
  ...
  **Eight antagonist rounds, every one of which found something.** Full record in
  ...
  > actionable from the kalender is **E5-05**; the minimumdoel level is **E5-04** (its input exists since E1-12 and
  > E1-21; computing it is E5-04's, owner ruling 2026-09-13). An
...
- [ ] **E5-04 — Minimumdoel-level coverage (inspection level)**
  ...
  *Unblocked 2026-09-13:* E1-12 loads the 998 decreed minimumdoelen and E1-21 the 5,835 G goals of Op.stap snapshot 1.2, 4,983 of them concorded to 992 minimumdoelen. By owner ruling of that day this story now owns the clause "minimumdoel-level coverage returns real results" that E1-12, E1-21, E1-03 and E1-04 carried; they close on the input. *Carry forward:* the six minimumdoelen no G goal concords (`6-7.1.6`, `K-1.2.6`, `4-2.2.23`, `6-2.2.3`, `6-6.2.5`, `6-6.3.9`, ADR-0032 decision 5) must not be presented as gaps the teachers left, and on the API path a doelsoort filter "only MD" matches nothing: coverage reads `minimumdoelRef`, not the doelsoort.
...
- [x] **E5-06 — Export coverage overview (proof of coverage)** — *built 2026-08-06 on `story/E5-06-dekking-export`
  ...
  *Done when:* an export reproduces the on-screen coverage faithfully. Ref: FR-9.5, FR-11.2.
  ...
  > rendered the level the kopblok declares absent two rows above** (Art. V.2, E5-04, then blocked on E1-12), which is the
