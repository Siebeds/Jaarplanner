...
> ticket en krijgt er ook geen: wie aan een story werkt, volgt de `jaarplan-build`-flow zoals voorheen.
...
Maak een ticket nooit met de hand aan maar met `tickets.mjs nieuw` (zie onder): dan klopt het nummer en staat de
structuur er al.
...
| `bijgewerkt` | `JJJJ-MM-DD UU:MM`, bij **elke** wijziging bijgewerkt: het bord gebruikt dit om te kiezen welke versie de nieuwste is | ja |
...
- **Technisch:** Aanleiding, Voorgestelde wijziging, Acceptatiecriteria, Buiten scope, Werklog.
...
**In review** is geen status die iemand schrijft. Het bord zet een kaart daar als de eindstatus van de agent
(`te-testen` of `klaar`) al op een branch of in een worktree staat, maar nog niet op `main`: het werk is af en wacht op
jouw merge. Na de merge en een `git pull` staat de status op `main` en schuift de kaart vanzelf door.
...
- **Functioneel architect:** maakt tickets aan met de skill `ticket-aanmaken`, op `main`, en zet ze op
  `klaar-voor-bouw` als ze verfijnd zijn. Pas een ticket dat `in-uitvoering` is niet inhoudelijk aan: dat botst met de
  branch van de agent. Aanvullingen worden een nieuw ticket, of worden eerst met de eigenaar besproken.
...
nog een open beslissing is, dan wordt dat een open vraag en blijft het ticket liggen tot de eigenaar beslist.
...
Draai ze vanuit de map van je checkout; ze passen alleen bestanden aan en committen niets.

| Opdracht | Wat het doet |
| --- | --- |
| `node tools/backlog-board/tickets.mjs lijst [--status <kolom>] [--soort FB\|TB]` | alle tickets zoals het bord ze ziet |
| `node tools/backlog-board/tickets.mjs check [bestand ...] [--alle]` | controleert de structuur (in deze checkout, of met `--alle` het hele bord) |
| `node tools/backlog-board/tickets.mjs nummer FB\|TB` | het volgende vrije nummer |
| `node tools/backlog-board/tickets.mjs nieuw FB\|TB --titel "..." --door <wie>` | nieuw ticket met de vaste structuur; opties `--prioriteit`, `--status`, `--branch`, `--fr` |
| `node tools/backlog-board/tickets.mjs status <id> <status> --door <wie>` | status wijzigen; opties `--branch`, `--pr`, `--log "..."` |
| `node tools/backlog-board/tickets.mjs log <id> --door <wie> "..."` | een regel in het werklog |
| `node tools/backlog-board/tickets.mjs blokkeer <id> --door <wie> "reden"` / `deblokkeer <id> --door <wie>` | geblokkeerd aan of uit |
| `node tools/backlog-board/tickets.mjs pr <id> <nummer> --door <wie>` | het PR-nummer invullen |

Elke wijziging via de CLI zet `bijgewerkt` en schrijft een werklogregel. Wie een ticket toch met de hand aanpast,
moet `bijgewerkt` zelf bijwerken, anders kan een oudere versie op een andere branch het halen.
