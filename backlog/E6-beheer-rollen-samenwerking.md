...
- [x] **E6-02 — Role-based authorization (the ADR-0030 matrix)** — **Closed 2026-09-14** on `feature/e6-rollen-rechten`, built with E6-04 in four slices, each through the test-runner and the antagonist (worklogs in `worklogs/E6-02/`): the rights model and one rights service (slice 1), the matrix enforced server-side on every write route with a sweep test and the wizard's own write actions (slice 3), and every control shown only to whoever holds its right (slice 4). Five owner defaults were added to Art. VI.1 on the way (I24–I28, with the Q4 and Q5 clarifications). *roles ruled by the owner on 2026-09-11, 2026-09-13 and 2026-09-14, and **ratified into Art. VI.1 on 2026-09-14**; in progress since 2026-09-13 on `feature/e6-rollen-rechten`, **built and delivered together with E6-04** (ADR-0030 R12).* Its first task, part 1 of the Art. XI amendment, is done: drafted on `story/E6-02-amendering`, audited in four rounds, and ratified on 2026-09-14.
  ...
  *The three carry-forwards above (E2, E3-01, E3-07) are met since E6-02 slice 3 (2026-09-14, `story/E6-02-afdwingen`): every write route carries its §3 row, the five jaarplan write routes as one unit under the klas-planning row, and a sweep test fails for any write route a gebruiker without rights can reach. Kept rather than deleted, as the record of what the matrix had to cover.*
...
- [ ] **E6-03 — Schooljaren beheer-UI + wijzigen/verwijderen + rolpoort** — *narrowed 2026-07-29: the create/read API landed in E3-01*
  ...
  *Why this story was narrowed rather than closed:* FR-12.1 names an **admin** doing this, and the E3-01 audit flagged that leaving E6-03's original wording would silently retire the FR's user-facing half on the strength of a server story — the exact failure that produced E1-13 and E1-14. What remains is genuinely the UI, update/delete, and the role gate.
  ~~*Carry-forward:* `POST /api/schooljaren` has **no role check**. Since E6-01 it requires a session, but any signed-in person may call it. Creating a school year is a beheerder action under the FA §3.2 matrix; apply it here.~~ *Met 2026-09-14 by E6-02 slice 3: `POST /api/schooljaren` and the klas create, edit and delete routes are directie-only. What this story still owes is the UI, update and delete.*

- [x] **E6-04 — Klassen + leerkrachten + rechten** — **Closed 2026-09-14** with E6-02 (slice 2): the Instellingen part *Gebruikers* for directie (invite by UPN, the directie right and themabeheer, klastoewijzingen, hoofdleerkracht appointments per schooljaar, removal), the last directie protected counting only directieleden who have signed in, and the Klassen part read-only for everyone else. *in progress since 2026-09-13 on `feature/e6-rollen-rechten`, **built and delivered together with E6-02** (owner ruling 2026-09-13).* The owner's rulings are ratified (2026-09-14): several leerkrachten per klas and several klassen per leerkracht (ADR-0030 R15); no separate ICT role, and directie may give the directie right to someone else (R16).
...
- [x] **E6-08 — Colleagues view each other's plans (read, per rights)** — *done by FB-013 ([ADR-0040](../docs/adr/0040-klassen-inkijken-per-jaarfase.md))*
...
- [!] **E6-09 — Visibility scope** — *blocked: Art. XIV teacher visibility*
  ...
  *Seam built by FB-013:* the one matrix row `Rechtenmatrix.KlasplanningBekijken` ([ADR-0040](../docs/adr/0040-klassen-inkijken-per-jaarfase.md)). What stays `[!]` is directie's answer, and a setting only if they want one.
