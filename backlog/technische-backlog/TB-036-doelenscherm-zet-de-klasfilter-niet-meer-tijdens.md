...
- [ ] Gegeven een leerkracht met een klas, wanneer ze naar Doelen gaat, dan toont de browserconsole geen
  "Cannot update a component while rendering"-waarschuwing.
- [ ] Gegeven dezelfde leerkracht, dan staat de jaar/fase-filter bij binnenkomst op die van haar klas, zoals nu.
- [ ] Gegeven een andere klas in de selectie, wanneer ze wisselt, dan volgt de filter de nieuwe klas, zonder
  tussentijds een lege filter te tonen.
- [ ] De bestaande tests van `features/doelen` en `state/doelenfilter` blijven groen, met een test voor het wisselen.
