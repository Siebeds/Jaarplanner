...
- [~] **E2-08 — Trigger the matching: an invocation surface for FR-4.1** — *added 2026-07-28 (third antagonist audit): E2's headline feature is unreachable; **claimed 2026-07-29**, in progress on `feature/e2-ai-matching`*
  ...
  *Left undone on purpose, recorded here rather than only in the worklog (implementation 2026-07-29):*
  1. **A substitution loses which code the AI originally proposed.** `DoelKoppeling.VervangLeerplandoel` overwrites `LeerplandoelCode` and clears `AiMotivatie` (the motivation argued for the *old* goal, so carrying it over would present an AI justification for something the AI never suggested — Art. IV.3). The consequence is that afterwards nothing distinguishes an overridden AI suggestion from a purely manual link. Retaining it needs a nullable `VervangenLeerplandoelCode` column and therefore a migration, which was judged out of proportion to the story; **decide whether that audit trail is wanted** before any export claims to show how a coupling came about (E7/FR-11). *Also filed under E5-07/E5-06, which are the stories that would have to act on it — a deferral recorded only here is one that gets lost.*
  2. **`LeerdoelSelectie.Alles` puts every loaded leerplandoel in the prompt.** Safe today — the database holds only the demo seeder's `DEMO-*` goals and no Op.stap import can be triggered yet — but **the first real per-discipline import (E1-05/E1-15) makes the matching prompt grow with the curriculum.** No cap was invented here on purpose: choosing which goals are silently withheld from the model is a pedagogical decision, not a coding one. The endpoint's optional `selectie` and the UI's two filters are the lever that exists; whoever ships the real import should weigh whether a default narrower than "alles" is needed at that point. *Also filed under E1-15 (the story that must act) and in E7-11's route enumeration (an uncapped candidate list behind an anonymous, billable endpoint).*
  3. **Both "aanpassen" readings ship side by side.** The pre-existing status-to-`manueel` button (override the verdict) and the new substitution (replace the doel) are both offered, because directie has not ruled which one FR-4.3 means (E2-05 note above). Whichever way it lands, one of the two is removable without rework — but until then the review row carries two controls a teacher could confuse. Both now carry explanatory copy (`matching.manueelUitleg`, `matching.vervangenUitleg`), because they land on the same visible `Manueel` badge and count identically for dekking.
  4. **Substitution cannot leave the link undecided — "change the doel" and "accept it into my dekking" are fused (recorded 2026-07-29, antagonist audit).** `DoelKoppeling.VervangLeerplandoel` unconditionally sets `Status = Manueel`, and `OngekoppeldeDoelenQuery` counts `Manueel` as *gedekt* immediately, so a teacher who only wants to correct the code cannot keep the koppeling `Voorgesteld` for later. That is a **conflation, not an auto-apply** — the act is teacher-initiated and now disclosed on screen — so it was ruled not a breach and **the behaviour is deliberately left as is**: the FR-4.3 ruling above may delete one of the two controls anyway, and building a third status path first would be work thrown away. **The FR-4.3 ruling settles this too**; if directie keeps both controls, decide whether substitution should preserve `Voorgesteld`.
