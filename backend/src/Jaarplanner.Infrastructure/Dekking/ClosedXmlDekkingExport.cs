...
public sealed class ClosedXmlDekkingExport : IDekkingExport
{
    ...
    private static void SchrijfTabel(IXLWorksheet sheet, DekkingWeergave dekking, int kopregel)
    {
        ...
        foreach (var doel in dekking.Doelen)
        {
            ...
            // one name. Empty exactly when the doel is not covered, which is the payload's own guarantee over the three
            // lists together. The thema's come first and each fiche says it is one (owner ruling, 2026-09-11): in a
            // document handed to an inspecteur, "Turnen" alone would read as the name of a thema. An own activiteit says so too (ADR-0049).
            sheet.Cell(rij, (int)DekkingKolom.DekkendeThemas).SetValue(string.Join(
                "; ",
                doel.DekkendeThemas
                    .Concat(doel.DekkendeFiches.Select(fiche => $"{fiche} (algemene fiche)"))
                    .Concat(doel.DekkendeActiviteiten.Select(activiteit => $"{activiteit} (eigen activiteit)"))));
