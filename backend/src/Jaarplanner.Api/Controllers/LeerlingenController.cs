...
/// reading a klas (<c>KlasplanningBekijken</c>) does not reach a child (R17), so the list is <c>OntwikkelingsrapportLezen</c> (admin, and the klas's own K3
/// leerkrachten with no end date). Every write is <c>LeerlingenBeheren</c> (admin, and those leerkrachten only during
