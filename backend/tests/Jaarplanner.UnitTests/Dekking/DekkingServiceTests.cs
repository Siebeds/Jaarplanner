...
public sealed class DekkingServiceTests
{
    ...
    [Fact]
    public async Task Een_minimumdoel_is_gedekt_wanneer_een_aanvaarde_plaatsing_het_draagt()
    {
        var service = Maak(
            plaatsingen: [Plaatsing(HerfstId, "Herfst", KoppelingStatus.Aanvaard)],
            doelen: [Doel("NAT-K3-01")],
            themaMinimumdoelen: [Op("K-1", HerfstId, "Herfst")]);

        var dekking = await service.BerekenAsync(KlasId);

        Assert.True(Mdvan(dekking, "K-1").IsGedekt);
        Assert.False(Mdvan(dekking, "K-2").IsGedekt);
    ...
    public async Task Een_voorgestelde_of_geweigerde_plaatsing_dekt_niets(KoppelingStatus status)
    {
        ...
        // can prove to an inspectie (Art. IV.1). A rejected one is an answer.
        var service = Maak(
        ...
        Assert.False(Mdvan(dekking, "K-1").IsGedekt);
    ...
    public async Task Een_ingepland_thema_dekt_geen_leerplandoel()
    {
        ...
        var service = Maak(
            plaatsingen: [Plaatsing(HerfstId, "Herfst", KoppelingStatus.Aanvaard)],
            doelen: [Doel("NAT-K3-01", minimumdoelRef: "K-1")],
            themaMinimumdoelen: [Op("K-1", HerfstId, "Herfst")]);

        var dekking = await service.BerekenAsync(KlasId);

        Assert.True(Mdvan(dekking, "K-1").IsGedekt);
        var doel = Doelvan(dekking, "NAT-K3-01");
    ...
    public async Task Een_vervallen_plaatsing_dekt_niets_en_maakt_het_cijfer_onbetrouwbaar()
    {
        ...
        // The totals are WITHHELD, not merely flagged. A number beside a false flag is a number a caller renders.
    ...
    public async Task Een_vervallen_maar_geweigerde_plaatsing_maakt_het_cijfer_niet_onbetrouwbaar()
    {
        ...
        // exists, so its staleness can never change the figure.
        var service = Maak(
            plaatsingen:
            [
    ...
    {
        // The other side of the same judgement call: a `voorgesteld` placement may still be accepted, and accepting
        // it would raise the figure. It is undecided, not discarded.
        var service = Maak(
    ...
    {
        // The subdoel and activiteit layers hang off a subthema at a leeftijd. The service cannot do that filtering
        // itself (it is a database concern), so what it owes is passing the klas through.
        var opslag = new FakeDekkingOpslag([Doel("NAT-K3-01")]) { Leerjaar = KleuterLeerjaar };
        ...
        Assert.Equal(KlasId, opslag.GevraagdeKandidaatKlasId);
    }

    [Fact]
    public async Task Hetzelfde_thema_twee_keer_geplaatst_telt_een_keer()
    {
        // A thema may legitimately be placed in two periods (Art. IX.3 allows a list per block, and a school may
        // revisit a thema). Coverage is about the doel being taught, not about how often.
        var service = Maak(
            plaatsingen:
            [
                Plaatsing(HerfstId, "Herfst", KoppelingStatus.Aanvaard),
        ...
        Assert.Equal(1, dekking.AantalMinimumdoelenGedekt);
        Assert.Equal(["Herfst"], Mdvan(dekking, "K-1").DekkendeThemas);
    }

    [Fact]
    public async Task Een_doel_dat_twee_themas_dragen_noemt_ze_beide_alfabetisch_en_een_thema_maar_een_keer()
    {
        // The evidence half of Art. V: an export that claims coverage must be able to say through what. A thema that
        // carries the same ref twice (a fake may; the unique index forbids it in a database) is named once.
        var service = Maak(
            plaatsingen:
            [
            ...
            themaMinimumdoelen:
            [
                Op("K-1", WinterId, "Winter"),
                Op("K-1", HerfstId, "Herfst"),
                Op("K-1", HerfstId, "Herfst"),
            ]);

        var dekking = await service.BerekenAsync(KlasId);

        Assert.Equal(["Herfst", "Winter"], Mdvan(dekking, "K-1").DekkendeThemas);
        Assert.Equal(1, dekking.AantalMinimumdoelenGedekt);
    }
    ...
    public async Task Een_leeg_jaarplan_dekt_niets_en_is_geen_fout()
    {
        ...
        // unresolved.
        var service = Maak(plaatsingen: [], doelen: [Doel("NAT-K3-01"), Doel("NAT-K3-02")]);
    ...
    // ordinary app code; only the sanctioned import path sets it, through EF property metadata).
    ...
    public async Task Een_onbekende_status_dekt_niets_en_blijft_als_onopgelost_gelden()
    {
        ...
        // value is reachable if the enum ever grows and this code is not updated.
        var service = Maak(
    ...
    public async Task De_minimumdoelref_reist_mee_zonder_het_minimumdoel_te_dekken()
    {
        ...
        var service = Maak(
            plaatsingen: [],
            doelen: [Doel("MD-01", minimumdoelRef: "K-1")],
            subthemas: [new Subthemakoppeling("MD-01", "Herfst", "Bladeren", IsIngepland: true)]);

        var dekking = await service.BerekenAsync(KlasId);

        Assert.Equal("K-1", Doelvan(dekking, "MD-01").MinimumdoelRef);
    ...
    public async Task De_klas_en_het_schooljaar_komen_uit_het_plan_zelf()
    {
        ...
        var service = Maak(plaatsingen: [], doelen: []);
    ...
    [Fact]
    public async Task Een_minimumdoel_in_een_voorgesteld_geplaatst_thema_wacht_op_een_beslissing()
    {
        var service = Maak(
        ...
        var md = Mdvan(await service.BerekenAsync(KlasId), "K-1");

        Assert.False(md.IsGedekt);
        Assert.Equal(Lacuneoorzaak.WachtOpBeslissing, md.Oorzaak);
        Assert.Equal(["Herfst"], md.KandidaatThemas);
    }

    [Fact]
    public async Task Een_minimumdoel_waarvan_het_thema_nergens_staat_is_niet_ingepland()
    {
        var service = Maak(
            plaatsingen: [],
            doelen: [],
        ...
        Assert.Equal(Lacuneoorzaak.NietIngepland, md.Oorzaak);
        Assert.Equal(["Herfst"], md.KandidaatThemas);
    }
    ...
    [InlineData(KoppelingStatus.Geweigerd)]
    public async Task Een_thema_dat_in_geen_enkele_periode_meer_staat_maakt_de_lacune_niet_ingepland(KoppelingStatus status)
    {
        // A stale placement is drawn in no period column whatever its status, so "sits in no period" is true of it and
        // the remedy is the same as for a thema never placed. The rejected, non-stale placement is its own cause below.
        var service = Maak(
            plaatsingen: [Plaatsing(HerfstId, "Herfst", status, isVervallen: true)],
            doelen: [],
            themaMinimumdoelen: [Op("K-1", HerfstId, "Herfst")]);
    ...
    {
        // The card stays visible in its period column, and the remedy is Weigering terugdraaien on it, a different
        // action from placing the thema, so it is a different cause (antagonist ronde 1, MAJOR-1, 2026-08-19).
        var service = Maak(
        ...
        Assert.Equal(Lacuneoorzaak.PlaatsingGeweigerd, md.Oorzaak);
        Assert.Equal(["Herfst"], md.KandidaatThemas);
    }

    [Fact]
    public async Task Een_open_voorstel_gaat_voor_op_een_weigering_elders_en_noemt_alleen_dat_thema()
    {
        // Cheapest route wins: a teacher told to undo a rejection when a single "Aanvaarden" click would do is being
        // sent the long way round. Only the thema of that cause is named.
        var service = Maak(
            plaatsingen:
            [
            ...
            [
                Op("K-1", HerfstId, "Herfst"),
                Op("K-1", WinterId, "Winter"),
                Op("K-1", Guid.NewGuid(), "Lente"),
            ]);

        var md = Mdvan(await service.BerekenAsync(KlasId), "K-1");

        Assert.Equal(Lacuneoorzaak.WachtOpBeslissing, md.Oorzaak);
        Assert.Equal(["Winter"], md.KandidaatThemas);
    }
    ...
    public async Task Een_gedekt_doel_heeft_geen_oorzaak_en_geen_kandidaten()
    {
        ...
        // the same value. The candidate list is non-empty in the fixture, so the covered branch really ignores it.
        var service = Maak(
            plaatsingen: [],
            doelen: [Doel("NAT-K3-01")],
    ...
    public async Task Een_leerplandoel_wacht_nooit_op_een_themaplaatsing()
    {
        ...
        foreach (var status in new[] { KoppelingStatus.Voorgesteld, KoppelingStatus.Geweigerd })
        {
            var service = Maak(
                ...
                doelen: [Doel("NAT-K3-01")],
                kandidaten: [Kandidaat("NAT-K3-01", HerfstId, "Herfst")],
                subthemas: [new Subthemakoppeling("NAT-K3-01", "Herfst", "Bladeren", IsIngepland: false)]);
    ...
    {
        // The state right after a subdoel was proposed and nobody answered it. Planning would not help, because only
        // aanvaard/manueel links count (Art. V.1), so this cause routes to the thema screen and not to the kalender.
        var service = Maak(
            plaatsingen: [Plaatsing(HerfstId, "Herfst", KoppelingStatus.Aanvaard)],
            doelen: [Doel("NAT-K3-01")],
    ...
    [Fact]
    public async Task Een_besliste_koppeling_verslaat_een_onbesliste_op_hetzelfde_doel()
    {
        // The same subthema carries the goal both as a decided link and as an undecided one, which the storage reads
        // return as two rows on purpose. NietIngepland is the honest answer: the decided link already exists, so
        // telling the teacher to go decide a link would send them to a screen where the work is done.
        var service = Maak(
            plaatsingen: [],
            doelen: [Doel("NAT-K3-01")],
            kandidaten:
    ...
    public async Task Het_vooruitzicht_en_de_dekking_tellen_dezelfde_leerplandoelen()
    {
        ...
        var service = Maak(
            ...
            kandidaten: [Kandidaat("NAT-K3-02", WinterId, "Winter")],
            subthemas:
            [
    ...
    /// tests down the <i>fallback</i> path instead. 0 is the kleutergroep, which matches the plan's own "K3 derde
    /// kleuterklas", the <see cref="Doel"/> helper's <c>K3</c> jaar/fase and the mijlpaal <c>K-</c> of the two
    /// minimumdoelen in store.
    /// </para>
    /// </summary>
    private static DekkingService Maak(
        ...
        new(
            ...
            new FakeDekkingOpslag(doelen)
            {
                ...
                // default: a school with no linked content is exactly the state in which nothing accounts for a goal.
                Kandidaten = kandidaten ?? [],
                ThemaMinimumdoelen = themaMinimumdoelen ?? [],
                Subthemakoppelingen = subthemas ?? [],
                Minimumdoelen = [Md("K-1"), Md("K-2")],
            });

    private static Minimumdoel Md(string minimumdoelRef) =>
        new(minimumdoelRef, "K-", minimumdoelRef[2..], $"Tekst van {minimumdoelRef}");

    private static Themaminimumdoelkoppeling Op(string minimumdoelRef, Guid themaId, string themaNaam) =>
        new(minimumdoelRef, themaId, themaNaam);

    private static MinimumdoelDekking Mdvan(DekkingWeergave dekking, string minimumdoelRef) =>
    ...
    /// <summary>
    /// One candidate link for the gap-analyse: a subdoel or activiteit link of a subthema of the thema.
    /// <c>isBeslist</c> defaults to <c>true</c> because a decided link is the ordinary case: an undecided one is a
    /// specific state, and a test about it should have to say so.
    /// </summary>
    ...
    private static (DekkingService Service, FakeDekkingOpslag Opslag) MaakMetFiches(
        ...
        IReadOnlyList<Subthemakoppeling>? subthemas = null)
    {
        var opslag = new FakeDekkingOpslag(doelen)
        {
            Leerjaar = KleuterLeerjaar,
    ...
    public async Task Een_geplande_algemene_fiche_dekt_een_doel_ook_zonder_enig_geplaatst_thema()
    {
        ...
        // The fiche layer is asked for THIS class.
        Assert.Equal(KlasId, opslag.GevraagdeFicheKlasId);
    }

    [Fact]
    public async Task Een_doel_dat_subthema_en_fiche_dragen_telt_een_keer_en_noemt_beide()
    {
        var (service, _) = MaakMetFiches(
            ...
            subthemas: [new Subthemakoppeling("LO-K3-01", "Herfst", "Bladeren", IsIngepland: true)]);
    ...
    {
        // A link to a goal outside the denominator cannot push the figure past it.
        var (service, _) = MaakMetFiches(
            plaatsingen: [],
