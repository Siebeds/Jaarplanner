...
/// <summary>
/// Persistence of <see cref="Schooljaar"/> and its owned <see cref="Schoolvakantie"/> collection (E3-05,
...
public sealed class SchooljaarPersistentieTests : IAsyncLifetime
{
    ...
    public async Task Verwijderen_neemt_de_vakanties_mee()
    {
        ...
        schooljaar.VoegVakantieToe(new Schoolvakantie("Krokusvakantie", new DateOnly(2028, 2, 14), new DateOnly(2028, 2, 20)));
        ...
        await using (var context = _db.MaakContext())
        {
            ...
            var resterend = await context.Database
                .SqlQueryRaw<int>("SELECT COUNT(*)::int AS \"Value\" FROM schoolvakanties")
                .SingleAsync();
