...
.DESCRIPTION
    Builds the API first, then reads the connection string from Key Vault, opens the PostgreSQL firewall to this
    machine's public address for the duration of the run, and closes it again, also when the migration fails. The app
    never migrates itself at startup.
    ...
    The build runs before the connection string is read, and without build servers: MSBuild nodes and the compiler
    server outlive the command that started them, and would otherwise keep the database administrator's password in
    their environment after the script ends. The migration then runs with --no-build: dotnet-ef still evaluates both
    projects with MSBuild while the password is in the environment, but compiles nothing, and
    MSBUILDDISABLENODEREUSE keeps that evaluation off every reusable MSBuild node, another session's included.
...
$apiProject = Join-Path $backend 'src/Jaarplanner.Api'
$rule = 'migration-from-operator'

$csproj = Get-Content (Join-Path $apiProject 'Jaarplanner.Api.csproj') -Raw
...
# Before any secret is read. The build restores too, so a fresh checkout needs no separate dotnet restore.
...
try {
    # dotnet-ef's project evaluation must not hand the environment to an MSBuild node that outlives the script.
    $env:MSBUILDDISABLENODEREUSE = '1'
    # The design-time host starts the API. Outside Development it refuses to start without the Entra and Key Vault
    ...
    # --no-build: the build above is the one this migration uses (the Debug configuration, dotnet-ef's default).
    dotnet dnx "dotnet-ef@$efVersion" -- database update --no-build `
