...
- ~~**E3-08**'s zoom level and **E3-07**'s drag state are the first genuinely transient UI state, and are where Zustand returns per ADR-0014.~~ **Amended 2026-07-31 (owner ruling on the E3-08 audit): both live in plain component state, and Zustand has not returned.** They are `useState` in `Jaarplankalender`, because a module-scoped store outlives the component: it would carry one class's chosen grain into the next (the klas selector sits above the router outlet on the same route, so a class switch re-renders rather than remounts) and it leaks between tests. Each value has exactly one reader tree, rooted in the component that already owns the fetch. **This equally covers E3-07's drag state**, which took the same decision one day after this ADR was accepted, was never ratified, and was missed by every audit including its own. The full amendment is on [ADR-0014](0014-frontend-state-and-dnd.md#amendment-2026-07-31--the-kalenders-zoom-and-drag-state-live-in-component-state); struck through rather than deleted, because the record of what was decided first is the point of an ADR. E0-10 removes the placeholder sidebar store, which was a control that only toggled its own label.
- **The zoom is therefore not in the URL**, so unlike `?schooljaar=` and `?klas=` it does not survive a reload and cannot be shared. That is a consequence the owner accepted rather than an oversight, and it is the one place this ADR's own clause 1 ("a screen worth navigating to is worth a URL") is knowingly not applied. Mirroring the tier into a `?niveau=` param is the obvious follow-up if a teacher ever needs to send a colleague the fine view; it needs its own story, because the mirror has to survive a class switch and a tier the server does not recognise.
