# ADR-0049 — A thema's doelsuggesties propose minimumdoelen as themadoelen
...
- **D1.** An accepted proposal stays stored as `aanvaard` beside the themadoel it made, a rejected one as `geweigerd`.
  A run proposes no minimumdoel that is already a themadoel or already has a proposal of any status. So a themadoel
  that was accepted and later unlinked is not proposed again either; a person can still link it by hand.
...
- **Domain.** `Minimumdoelsuggestie` (`Id`, `ThemaId`, `MinimumdoelRef`, `Status`, `AiMotivatie`) replaces the
  `DoelKoppeling` rows of `Thema.Doelsuggesties`, in a new table `thema_minimumdoelsuggesties` with a restricting
  foreign key to `minimumdoelen.Ref` and a unique index on `(ThemaId, minimumdoel_ref)`.
