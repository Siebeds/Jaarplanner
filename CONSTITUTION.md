...
The Op.stap goals are imported from **KOV's Op.stap API**, read by the backend only. The Excel mapping of VII.1 describes the older per-discipline route, which stays available but is no longer the documented source. The database remains the runtime truth: no screen and no dekkingscijfer ever waits on KOV.
