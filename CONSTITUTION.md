...
5. AI calls **always request structured JSON** (goal codes + one-line motivation; or thema's, each with a start week and a one-line motivation, for plan generation; or one rewritten text; or woordweb words, each with a one-line motivation; or subdoelplaatsingen, each goal code with an existing or a new subthema and a one-line motivation, and each new subthema with a name, an onderzoeksvraag and a length) and the response is **validated before use**.
