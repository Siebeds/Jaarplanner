...
- **Minimumdoelen** (`GET /agodi/onderwijsdoelen/opstap`): `ref` = `uniqueCode` verbatim (`K-1.3.9`), `leeftijd` = its prefix (`K-` / `4-` / `6-`), `nr` = `code`, `omschrijving` = the doelzin followed by the uitbreiding, as plain text. **Every** decreed minimumdoel is imported: the decree applies in full, whatever the goal-set scope below.
- **Leerplandoelen** (`…/snapshots/{versie}/krcItems`, E1-21): **only goal set G for now** (owner ruling 2026-09-11; P, S, +, A, Z and V are skipped), mapped to `Doelsoort.Gemeenschappelijk`, with the concordance in `minimumdoelRef`. Under that scope six minimumdoelen have no concorded goal left and can never be gedekt; a coverage view must say so rather than count them as a gap the teachers left (ADR-0032 decision 5).
