...
- **Architecture:** SPA over REST/JSON. Backend is pragmatically layered — `Domain` (entities, invariants, Dutch ubiquitous language) ← `Application` (use cases, AI orchestration, mapping) ← `Infrastructure` (EF Core, Excel import, AI clients). `Api` is thin. **This is a small app — favour clarity over ceremony; do not over-engineer.** One **background job** runs in the API process for the cat, one instance at a time ([ADR-0059](docs/adr/0059-de-kat-proactieve-agent.md)).
