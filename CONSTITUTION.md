...
- **Thema** *(school-scoped — shared school-wide)* — id, naam, invalshoeken?, `duurWeken` (≈ 4–6), `kernwoordenschat[]`, `rijkeWoordenschat[]`, subthema's[], themadoelen[], `doelsuggesties[]`. Shared through the thema-bibliotheek, and **edited by directie and by those who hold themabeheer** ([Art. VI.1](#article-vi--roles-privacy--security)).
  - **`doelsuggesties[]`** — the AI's **FR-4 proposals of a `Minimumdoel` as a themadoel** ([ADR-0049](docs/adr/0049-doelsuggesties-zijn-minimumdoelen.md)), each persisted as `voorgesteld` with `aiMotivatie`, for a minimumdoel of a mijlpaal that the thema's leeftijden meet (Art. V.1). Deliberately **distinct from the `themadoelen`**: a proposal awaits accept/reject (Art. IV); **accepting it makes its minimumdoel a themadoel**, and a rejected one is not proposed again. At thema level the AI proposes no leerplandoel: leerplandoelen reach a thema through its subthema's. **Only directie and themabeheer generate and review them** ([Art. VI.1](#article-vi--roles-privacy--security)), because an accepted proposal counts for the dekking of every klas that plans the thema.
