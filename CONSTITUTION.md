...
4. **Generate a year plan per class**: the AI proposes thema's on the free days of the school year.
5. **Edit it freely** in a calendar via drag-and-drop.
6. **Regenerate** the whole plan, keeping everything the teacher decided.
...
10. Be helped by **the cat**, an AI agent that notices on its own what needs attention in a klas and brings what helps: a **briefing** for a **vervanger**, **lesvoorbereidingen**, warnings about the dekking and activiteitvoorstellen, each decided by a person, and a chat about the tool ([ADR-0057](docs/adr/0057-vervanging-briefing-en-klasfiche.md) to [ADR-0060](docs/adr/0060-activiteitvoorstellen-op-een-aanbod-gat.md); the rules are Art. IV.8 and VI.1).
...
1. **The domain language is Dutch.** Domain entities and concepts use Dutch names in code: `Leerplandoel`, `Minimumdoel`, `Doelsoort`, `Thema`, `Subthema`, `Activiteit`, `Jaarplan`, `Themaplaatsing`, `Dekking`, `Concordantie`, `Klas`, `Schooljaar`. Translating *leerplandoel* vs *minimumdoel* loses meaning — do not translate domain terms.
...
3. **All user-facing strings are Dutch.** UI text is centralised in `frontend/src/i18n/nl.json`; never hard-code Dutch text in components. The language of a message follows **who it is for**, not which layer produced it:
   - **A message a teacher or an admin can act on is Dutch.** A validation error, a per-row import problem, a reason a placement was refused. It may be **generated server-side**: not every Dutch string a user reads has to live in `nl.json`, because a diagnostic that names a row number, an offending column or a specific thema cannot be assembled from a static catalogue without inventing a parallel one.
...
1. **The AI proposes; the human decides.** Nothing is "final" without teacher confirmation. "Teacher" here means the person who holds the right to decide that output under [Art. VI.1](#article-vi--roles-privacy--security): for a generated plan, a leerkracht of the klas, or an admin; for a thema's **doelsuggesties**, an admin or themabeheer, who need not teach any klas that plans the thema ([ADR-0030](docs/adr/0030-rollen-en-rechten-in-de-app.md) R14); for an AI **rewrite** of a text in an ontwikkelingsrapport, a leerkracht of the klas during its schooljaar, or an admin ([ADR-0035](docs/adr/0035-ontwikkelingsrapport-derde-kleuter.md) §3.5); for a word the AI proposes in a **woordweb**, the web's owner, or an admin ([ADR-0043](docs/adr/0043-eigen-woordweb-per-subthema.md)); for a **subdoelplaatsing**, a proposal to place a leerplandoel as subdoel in an existing or a new subthema, a hoofdleerkracht of that subthema's jaarfase, or an admin ([ADR-0050](docs/adr/0050-ai-plaatst-leerplandoelen-in-subthemas.md)); for an **activiteitvoorstel**, an activiteit the AI proposes under a subthema, the person who asked for it while she may create an own activiteit there, or an admin ([ADR-0056](docs/adr/0056-ai-stelt-activiteiten-voor.md)); for a goal the AI proposes for an **activiteit**, whoever may link that activiteit's goals: for a shared one a hoofdleerkracht of its jaarfase or an admin, for an own one its owner or an admin ([ADR-0054](docs/adr/0054-ai-zoekt-doelen-bij-een-activiteit.md)); for a **lesvoorbereiding**, a leerkracht of the klas or an admin, never a vervanger, who only reads it ([ADR-0058](docs/adr/0058-lesvoorbereiding-per-plaatsing.md)); for an activiteitvoorstel the cat brings on an **aanbod-gat**, a leerkracht of the klas or an admin ([ADR-0060](docs/adr/0060-activiteitvoorstellen-op-een-aanbod-gat.md)); for a **hoekverrijkingsvoorstel**, what the AI proposes to put in one hoek of a klas while a subthema runs, whoever may plan that klas, or an admin ([ADR-0070](docs/adr/0070-ai-stelt-hoekverrijkingen-voor.md)). A proposal the cat brings unasked (IV.8) is decided by the same person as one that was asked for. A human decides, and nothing is applied without that decision. **An answer in the cat's chat decides nothing and changes nothing:** it informs, and whatever it proposes is a proposal of one of the flows above ([ADR-0059](docs/adr/0059-de-kat-proactieve-agent.md)).
2. Every AI output (goal match, generated plan) must be **reviewable and accept/reject/adjustable**, and its **status must be persisted** (`voorgesteld` / `aanvaard` / `geweigerd` / `manueel`). *Except a proposed thema placement in a jaarplan:* rejecting it removes the proposal, so no `geweigerd` placement is stored ([ADR-0053](docs/adr/0053-themaplaatsing-met-eigen-datums.md)).
   A thema's **doelsuggestie**, the proposal of a minimumdoel as themadoel, is accepted or rejected and not adjusted: whoever wants another minimumdoel rejects the proposal and links that one by hand ([ADR-0052](docs/adr/0052-doelsuggesties-zijn-minimumdoelen.md)).
   ...
   *Except a chat answer of the cat* ([ADR-0059](docs/adr/0059-de-kat-proactieve-agent.md)): it decides and changes nothing, carries no status and is not kept (VI.2). A proposal it starts goes through one of the IV.1 flows and is persisted there.
3. **Every suggestion carries a short motivation** ("waarom past dit doel hier?") surfaced in the UI (FR-4.2, Art. 7 of the FA). *Except a rewrite (ADR-0035 R24):* the teacher sees the old and the new text side by side, and no motivation is asked for. *Except a chat answer (ADR-0059):* it names the content of the tool it rests on instead.
4. **What the AI is sent, and what it may make up** ([ADR-0058](docs/adr/0058-lesvoorbereiding-per-plaatsing.md) L6). The AI is sent only the **school's own data** and the **loaded Op.stap goals**, **never** an external or unknown source, and the tool sends no pupil data of its own accord (Art. VI.2). Within that, it may make up **content** from its own knowledge: the words of a woordweb ([ADR-0043](docs/adr/0043-eigen-woordweb-per-subthema.md)), the name and onderzoeksvraag of a thema or subthema it proposes ([ADR-0050](docs/adr/0050-ai-plaatst-leerplandoelen-in-subthemas.md)), an activiteit's name, soort, expected outcomes and length ([ADR-0056](docs/adr/0056-ai-stelt-activiteiten-voor.md), [ADR-0060](docs/adr/0060-activiteitvoorstellen-op-een-aanbod-gat.md)), a lesvoorbereiding (ADR-0058), and the text of a hoekverrijking ([ADR-0070](docs/adr/0070-ai-stelt-hoekverrijkingen-voor.md)). **Every goal it names or links is one of the loaded goals it was sent**, never a goal, a code or a fact about the curriculum of its own; which goals are candidates is each feature's own rule, recorded in its ADR. Nothing it makes up is final before a person decides it (IV.1). **In the cat's chat** the question a gebruiker types goes as typed, and an answer about the school's content comes from the tool's own data, not from the model's knowledge (ADR-0059). **A rewrite** of an ontwikkelingsrapport text is narrower: only the teacher's own text goes to the AI (ADR-0035 R21), with the names of the klas's children replaced before the call (Art. VI.7), and it makes up nothing beyond rewording it.
5. AI calls **always request structured JSON** (goal codes + one-line motivation; or thema's, each with a start week and a one-line motivation, for plan generation; or one rewritten text; or woordweb words, each with a one-line motivation; or subdoelplaatsingen, each goal code with an existing or a new subthema and a one-line motivation, and each new subthema with a name, an onderzoeksvraag and a length; or activiteitvoorstellen, each with a name, an optional soort, its expected outcomes, a length in lesuren, the goal codes it works on, for the cat's proposals the subthema it goes under and the day and starting hour it proposes for it, and a one-line motivation; or a weekvoorstel, the klas's activiteiten it picks for one week, in order, each with the day it proposes and a one-line motivation, the hour being the tool's ([ADR-0067](docs/adr/0067-weekvoorstel-in-de-agenda.md)); or a hoekverrijking, one text for one hoek with a one-line motivation, or none; or a lesvoorbereiding, with its goal codes, instap, kern, afsluiting, materiaal, woordenschat, differentiatie, a duur in minutes and a one-line motivation; or a chat turn, with the answer and the content it rests on, or the lookup or proposal flow to run) and the response is **validated before use**.
...
8. **AI fits the school's goal-first authoring method**, not the reverse. The school builds a kennisrijk thema in ~10 steps (thema → themadoelen → subthema's → subdoelen → rijk aanbod). The **thema-opbouw wizard scaffolding these steps is a committed MVP feature** (not optional — see FA Bijlage A.7), with AI assist at **themadoel selection** (step 2) and **subdoel selection** (step 6, the matching of FR-4). On the thema page, once a thema has minimumdoelen as themadoelen, the AI may also propose where their leerplandoelen go: into an existing subthema of that leeftijd or into a new one (steps 4 to 6, [ADR-0050](docs/adr/0050-ai-plaatst-leerplandoelen-in-subthemas.md)); it works only from the themadoelen a person chose, so it does not run ahead of her. AI never skips ahead of the teacher in this flow. The wizard's user is a themabeheer holder, or an admin ([ADR-0030](docs/adr/0030-rollen-en-rechten-in-de-app.md) R29, R32), so "the teacher" in this clause is that person. The **brainstorm** (step 3) is a teacher's own **woordweb** on a subthema ([ADR-0043](docs/adr/0043-eigen-woordweb-per-subthema.md)); whether the wizard shows it too is E6-05's. The AI proposes words there only once the web holds a word of the teacher's own, so it does not run ahead of her either, and "the teacher" is the web's owner. At the **rijk aanbod** (step 7) the AI may propose activiteiten under a subthema, working on the subdoelen a person already chose for it ([ADR-0056](docs/adr/0056-ai-stelt-activiteiten-voor.md)); it runs only when she asks, and "the teacher" is the person who asked.
   **The cat is the one place where the AI acts unasked** ([ADR-0059](docs/adr/0059-de-kat-proactieve-agent.md)). A background job notices, without AI, what needs attention in a klas; the AI is then called unasked for two things only: the lesvoorbereidingen of a vervanging ([ADR-0058](docs/adr/0058-lesvoorbereiding-per-plaatsing.md)) and, five schooldagen before a thema starts in a klas, activiteitvoorstellen on the klas's **aanbod-gat** ([ADR-0060](docs/adr/0060-activiteitvoorstellen-op-een-aanbod-gat.md)). It prepares and never decides: what it brings is a proposal, decided as IV.1 says. On an aanbod-gat it goes past the goal-first order **for one klas**: it proposes own activiteiten on leerplandoelen in the klas's aanbod-gat, which are no subdoel there, leaves the subdoelen the hoofdleerkracht chose as they are, and what is accepted counts only where it is planned (Art. V.1).
...
1. **Dekking is computed, never stored**, per klas, in **two steps that are both shown**: the **dekkingsprognose**, what the school's thema's and subthema's aim at, and the **dekking**, what the klas's agenda holds. Only a decided link (status `aanvaard` or `manueel`) counts ([ADR-0047](docs/adr/0047-dekkingsprognose-en-dekking.md)).
   - A `Minimumdoel` is in the *dekkingsprognose* when it is a themadoel of at least one thema, and *gedekt* when such a thema is placed (`aanvaard` or `manueel`) in the klas's plan. **It counts only through a thema it is itself linked to**, never through a concorded leerplandoel. An accepted doelsuggestie counts only through the themadoel its acceptance made ([ADR-0052](docs/adr/0052-doelsuggesties-zijn-minimumdoelen.md)).
   - A `Leerplandoel` of the klas's jaar/fase is in the *dekkingsprognose* when it is a subdoel of, or linked to a shared activiteit of, a subthema at the klas's leeftijd. It is *gedekt* when that **subthema is placed in the klas's agenda** (placing only the thema above it is not enough).
   - A `Leerplandoel` linked to an **own activiteit** ([ADR-0049](docs/adr/0049-eigen-activiteit-van-de-leerkracht.md)) never counts through its subthema. It is in the *dekkingsprognose* of a klas at the activiteit's leeftijd that its owner teaches, and of a klas whose agenda holds the activiteit. It is *gedekt* when the **activiteit itself is planned in the klas's agenda** (at least one decided placement: one the AI proposed counts once it is accepted, [ADR-0067](docs/adr/0067-weekvoorstel-in-de-agenda.md)). The evidence names the activiteit as an own activiteit.
...
5. An admin can pull **school-wide and per-class** overviews from the beheerpagina (FR-9.4, FR-12).
...
1. **Rights, recorded in the app** *(owner rulings of 2026-09-11, 2026-09-13 and 2026-09-14, [ADR-0030](docs/adr/0030-rollen-en-rechten-in-de-app.md) §1)*. Only a **gebruiker** that an admin has added can log in, over the school's own Entra tenant. Microsoft Entra ID is the identity provider, and auth remains wrapped/swappable ([ADR-0011](docs/adr/0011-authn-authz-rbac-gdpr.md)). Which rights each gebruiker holds is recorded **in the app**, not in Entra. There are five rights, and one person may hold several.
   - **Admin** sees and edits everything. It adds gebruikers, links leerkrachten to klassen, appoints hoofdleerkrachten and gives themabeheer, which is what FA FR-12.2 has the beheerder do, and the beheerder is the adminrecht. An admin may give the adminrecht to someone else and take it away, so several gebruikers may hold it ([ADR-0061](docs/adr/0061-de-rol-directie-heet-admin.md)). **Admin is a right, not a function:** the directeur or an ICT-coördinator who runs the beheer holds it because an admin gave it, and is otherwise an ordinary gebruiker.
   - **Themabeheer** is held by a few named leerkrachten or zorgcoördinatoren. It edits thema's with their themadoelen and kernwoordenschat, and runs the FR-1 import of thema's and activiteiten, **including the goal links that import writes on themadoelen and subdoelen**. Only an admin may switch on the import's option to delete human decisions (`MenselijkeBeslissingenVerwijderen`), the one switch by which a re-import removes decided themadoelen and subdoelen that the file no longer carries. Themabeheer works the thema-opbouw wizard through all its steps. For a thema it builds there from scratch, and only through the wizard's own write actions, it also creates that thema's subthema's, subdoelen and activiteiten, at any leeftijd; changing existing subthema's stays with the hoofdleerkracht. Beside admin, it is the only right that has **doelsuggesties** generated, and the only one that accepts or rejects them.
   ...
   - **Leerkracht** is held through a **klastoewijzing** on one or more klassen. It is many-to-many: a klas may have several leerkrachten, such as a co-teacher or a duobaan. A leerkracht edits the planning of those klassen (jaarplan, (her)generatie, agenda, hoeken, algemene fiches) and can view other klassen. Every leerkracht with a klas at a leeftijd also edits the **content** of the shared activiteiten under the subthema's at that leeftijd, creates **own activiteiten** there (below), and edits those subthema's **streefwoordenschat**. They may delete a shared activiteit **they created themselves**, and only while no goal is linked to it (only an `aanvaard` or `manueel` link counts as linked, [ADR-0054](docs/adr/0054-ai-zoekt-doelen-bij-een-activiteit.md)). **By hand, subdoelen, goal links on shared activiteiten, creating a shared activiteit and deleting any other shared activiteit are for admins and that jaarfase's hoofdleerkrachten only**, because a goal link there counts for the dekking of every klas at that leeftijd that plans the thema. The only exceptions are themabeheer's: the FR-1 import, and the wizard for a thema it builds from scratch. The rest of the subthema stays with admins and the hoofdleerkrachten, except what the wizard creates for such a thema.
   - **Leerlingzorg** *([ADR-0035](docs/adr/0035-ontwikkelingsrapport-derde-kleuter.md) R18)* is given by an admin to a gebruiker, such as a zorgcoördinator. It reads every ontwikkelingsrapport (VI.7) and does nothing else.

   **An activiteit records its maker**, the gebruiker who created it. For a shared activiteit the maker only decides who may delete it: whoever created it may delete it while no goal is linked to it (a proposed or rejected goal does not count), whether or not they have a klas at that leeftijd, and also after the schooljaar. Activiteiten that predate this rule, and imported ones, have no maker and are purely shared.
   ...
   **An own activiteit is personal content** ([ADR-0049](docs/adr/0049-eigen-activiteit-van-de-leerkracht.md) E1 to E3). A new activiteit a leerkracht creates is her own: it belongs to her, not to her klas, and follows her across schooljaren. The leerkrachten of the same jaarfase read it and may use it, which gives them an own copy with the same content and goals; they do not edit the original. Its owner links its goals, and they count for the dekking of a klas once it is planned in that klas's agenda (V.1). The rest are defaults (ADR-0049 D1 to D9):
   - a hoofdleerkracht of that jaarfase and an admin still create shared activiteiten, and choose per new activiteit;
   - only a leerkracht with a klas at the leeftijd, or an admin, creates or uses an own activiteit;
   - its owner, the leerkrachten and hoofdleerkrachten of its leeftijd, and admins read it;
   - only its owner and an admin edit it, link its goals, move it, delete it or plan it;
   ...
   **Graadklassen, provisionally, until directie decides the Art. XIV graadklas question.** A klas states one jaarfase, so the leerkrachten of a graadklas get the leeftijd rights of that jaarfase only, and a hoofdleerkracht or an admin edits the other leeftijd's shared content. **The mapping from a klas to the leeftijden it grants rights for lives in one place in code**, so that directie's graadklas decision changes that place and nothing else (Art. XIV: an open decision sits behind a seam).

   **A gebruiker who holds none of the five rights**, such as a zorgcoördinator without themabeheer or Leerlingzorg, or a directeur or ICT-coördinator without the adminrecht and without a klas, can log in (only gebruikers an admin added can) but reads no klas's planning (below), apart from the klas of a running vervanging in which they are the vervanger. By default they can do nothing else ((e) below), apart from deleting an activiteit they created themselves (R33) and keeping a woordweb of their own (below). They create no own activiteit (ADR-0049 D2), and whether they may add personal subdoelen is E6-10's.

   **Who reads which klas's planning** (its jaarplan, agenda and dekking, and their export; [ADR-0040](docs/adr/0040-klassen-inkijken-per-jaarfase.md) Z1 to Z5). A leerkracht reads her own klassen and the klassen of her own jaarfase, of both jaarfasen when she teaches in two; a hoofdleerkracht reads the klassen of the jaarfase she is appointed for; themabeheer and admins read every klas; a vervanger reads the klas of her vervanging while it runs, without its export (below); anyone else reads none. Her jaarfase is that of a klas she teaches, or of her appointment, while its schooljaar has not ended (default Z7 below); the klas read may be of any schooljaar (default Z6 below), and its jaarfase comes from the one klas→leeftijden mapping above. Reading is not editing: only the klas's leerkrachten and admins edit its planning. One matrix row decides it, behind the E6-09 seam, so directie's answer on visibility (Art. XIV) changes that row only.

   **"Configurable" means two things, and only one of them is data.** *Who* holds which right is data, maintained by admins. *What* each right allows is one matrix, [ADR-0030 §3](docs/adr/0030-rollen-en-rechten-in-de-app.md#3-the-matrix-that-follows), which **supersedes the table in FA §3.2**. Changing a row there is a code change. The matrix is enforced server-side, as named policies declared in one place ([ADR-0011](docs/adr/0011-authn-authz-rbac-gdpr.md) §2). The ontwikkelingsrapport's rows are in it too ([ADR-0035](docs/adr/0035-ontwikkelingsrapport-derde-kleuter.md) §3.3), with Leerlingzorg as one more relation, and **the reading rule above does not reach them**: only the klas's leerkrachten, admins and Leerlingzorg read a child's report (ADR-0035 R17). One of those rows is the only place where admin has no ✓: only the K3 leerkrachten edit the K3 set of rapportdoelen and the scale, and an admin views them (ADR-0035 R31).

   **A row of that matrix is ratified by this article only as far as the rulings (R-items) it cites.** Citations count at column level too: every ✓ for admin rests on the ruling that admin sees and edits everything, and a "–" grants nothing. Whatever a row takes from an I-item of ADR-0030 §2, or from a lettered question of ADR-0030 §4, is a default and is **not** ratified here. The same holds for every item in the list of defaults below. *Directie's confirmation is outstanding* on visibility (Art. XIV, question 4 in [`docs/besluiten-gevraagd.md`](docs/besluiten-gevraagd.md)), graadklassen (question 14) and the role model as a whole (question 13).

   **A woordweb is personal content** ([ADR-0043](docs/adr/0043-eigen-woordweb-per-subthema.md) W2, W4). Every gebruiker may keep one woordweb of her own per subthema (default D2), and it follows her across schooljaren. Every signed-in gebruiker reads the words that stand in every woordweb (default D7); only its owner edits it, and an admin (default D3). It never counts for dekking.

   **An activiteitvoorstel is personal content too** ([ADR-0056](docs/adr/0056-ai-stelt-activiteiten-voor.md) A1 to A3). Whoever may create an own activiteit under a subthema may ask the AI to propose activiteiten there. The proposals are hers: she sees and decides them while she may still create an own activiteit at that leeftijd, and an admin sees and decides every leerkracht's proposals. An accepted proposal becomes an own activiteit of the person who asked, whoever decided it. An activiteitvoorstel the cat brings on an aanbod-gat is addressed to the klas's leerkrachten instead, and becomes the own activiteit of whoever accepts it ([ADR-0060](docs/adr/0060-activiteitvoorstellen-op-een-aanbod-gat.md)).

   **A vervanging gives a vervanger read-only access to one klas** ([ADR-0057](docs/adr/0057-vervanging-briefing-en-klasfiche.md) V1 to V5). An admin records that a leerkracht is absent from a klas, from a first day to a last day or until an admin ends it, and names a gebruiker as **vervanger**. It is not a klastoewijzing and not a sixth right. From the first day through the last the vervanger **reads** that klas's planning (its jaarplan, its agenda, including by default the own activiteiten planned there (ADR-0057 D3), and its dekking), its klasfiche, its lesvoorbereidingen and her briefing, and **edits and decides nothing**. The absent leerkracht keeps her rights, and she corrects the agenda afterwards: what the agenda holds for a past day, happened. A vervanger is a gebruiker of the school's Entra tenant like any other.

   **A lesvoorbereiding and a klasfiche follow the klas's planning** ([ADR-0058](docs/adr/0058-lesvoorbereiding-per-plaatsing.md), ADR-0057 V8): the klas's leerkrachten and an admin ask for, decide and edit them (defaults, ADR-0058 D1 and ADR-0057 D2); whoever reads the klas's planning reads a decided lesvoorbereiding and the klasfiche, and its vervanger reads both, open lesvoorbereidingen included.
   ...
   *Defaults and open questions, not rulings* (ADR-0030 §2 and §4, ADR-0040). Each is kept until the owner decides, or directie for visibility:
   ...
   - **I2:** statement 8's missing verb is read as "designates". Who gives themabeheer does not hang on it: an admin does, as FR-12.2 has the beheerder grant rights.
   ...
   - **ADR-0043 D2 to D5 and D7:** every signed-in gebruiker may keep a woordweb on any subthema; an admin edits every woordweb; deleting a subthema or a gebruiker deletes the woordwebs with it, except that the wizard's own subthema delete counts a woordweb as content its run did not create (I25); for I26, a woordweb under a thema counts as someone else's content, whoever made it; every signed-in gebruiker reads the standing words of every woordweb, and another person's open proposals and rejected words stay hers.
   - **ADR-0056 D1 to D9:** asking needs the right to create an own activiteit there, so a hoofdleerkracht without a klas at that leeftijd does not ask; at most five proposals a request, set in the configuration; a new request replaces the asker's open proposals unless it keeps none; a rejected or existing name is not proposed again; only the subthema's decided subdoelen are candidates; a proposal is 1 to 4 lesuren; an edited acceptance is stored as `manueel`; a proposal goes with its subthema and its asker; a colleague neither sees nor decides another's proposals.
   - **ADR-0057 D1 to D8:** a vervanging is its own entity, so nothing a klastoewijzing grants reaches the vervanger; the vervanger is a column of reading rows only, reads the briefing and not the terugkeerbriefing, and does not export; she reads an own activiteit only as far as the klas's agenda holds it; two vervangingen of one klas may overlap; the briefing looks two weeks back and five schooldagen ahead; the klasfiche is one per klas and starts empty.
   - **ADR-0058 D1 and D2, ADR-0060 D2:** a vervanger reads open lesvoorbereidingen too; a plaatsing has at most one open and one decided lesvoorbereiding; the cat's activiteitvoorstel on an aanbod-gat is decided for all the klas's leerkrachten once one of them decides it.
   ...
   - **I13:** re-scoping a subthema to another leeftijd needs the hoofdleerkracht right at both leeftijden, or admin.
   - **I15:** the activiteit fields the owner's answer did not name (type, onderzoeksvraag, kleur, lengte) are content too.
   - **I16:** the subthema fields other than streefwoordenschat (naam, duur, probleemstelling, onderzoeksvragen) follow the subthema itself: admins and the hoofdleerkrachten.
   ...
   - **I22:** the server tells a wizard write from a hand write through wizard-only write actions, which admit themabeheer and admin, and only for a new thema. Themabeheer gets no right on the ordinary subthema, subdoel and activiteit routes, apart from the maker's delete right (R33).
   ...
   - **I24:** a wizard run ends when themabeheer or an admin finishes or closes it, or 14 days after the wizard's last write action in it, whichever comes first. From then on its thema is no longer new (I23) and the ordinary rights apply.
   ...
   - **I26:** a themabeheer holder deletes a thema only when it holds no subthema, subdoel or activiteit other than what its own open wizard run created, an activiteit of that run carrying a goal link not counting as the run's (I27); any other thema only an admin deletes. A thema placed in a jaarplan is deleted by nobody, as before.
   ...
   - **(c):** in a jaarfase with no hoofdleerkracht, only an admin does, by hand, what the hoofdleerkracht would: its subthema's, subdoelen, goal links, and the deletions only a hoofdleerkracht may make.
   - **(e):** a zorgcoördinator, or any gebruiker holding none of the five rights, may do nothing beyond themabeheer and Leerlingzorg, if given, apart from the maker's delete right (R33) and keeping a woordweb of their own (ADR-0043 D2); without a right they read no klas's planning (above), apart from the klas of a running vervanging.
2. **No pupil personal data in the MVP** (GDPR/AVG), **except the ontwikkelingsrapport under VI.7**. Staff accounts only: no pupil ever logs in. The free text a gebruiker writes outside the rapport, a **klasfiche** and a question in the **cat's chat** above all, is about the klas and the tool: both screens say in visible text that no name or information about a child belongs there, a briefing names where a vervanger gets that information instead, and the tool keeps no chat conversation ([ADR-0057](docs/adr/0057-vervanging-briefing-en-klasfiche.md), [ADR-0059](docs/adr/0059-de-kat-proactieve-agent.md)).
...
7. **The ontwikkelingsrapport (K3 only)** *([ADR-0035](docs/adr/0035-ontwikkelingsrapport-derde-kleuter.md) §1; entities in Art. IX.4)* is the one place pupil data lives.
   ...
   - **Who:**
     ...
     - every K3 leerkracht edits the one K3 set of rapportdoelen and the one scale (R4 to R6). An admin does not edit them, but can view them (R31). That is the one exception to *admin sees and edits everything* (VI.1).
   ...
   - **Deletion:** kept until an admin wipes a schooljaar (R19), and the beheerpagina shows which schooljaren still hold pupil data and reminds the admins of them (R28). A leerling can be deleted with their reports at any time (default D8).
...
- **Architecture:** SPA over REST/JSON. Backend is pragmatically layered — `Domain` (entities, invariants, Dutch ubiquitous language) ← `Application` (use cases, AI orchestration, mapping) ← `Infrastructure` (EF Core, Excel import, AI clients). `Api` is thin. **This is a small app — favour clarity over ceremony; do not over-engineer.** The cat's **background job** runs one instance at a time, by default in the API process ([ADR-0059](docs/adr/0059-de-kat-proactieve-agent.md)).
...
- **Thema** *(school-scoped — shared school-wide, limited to its leeftijden)* — id, naam, invalshoeken?, `duurWeken` (≈ 4–6), `leeftijden[]`, `kernwoordenschat[]`, `rijkeWoordenschat[]`, subthema's[], themadoelen[], `doelsuggesties[]`. Shared through the thema-bibliotheek, and **edited by admins and by those who hold themabeheer** ([Art. VI.1](#article-vi--roles-privacy--security)).
  - **`leeftijden[]`** — one or more of the nine jaar/fase codes, by default all nine ([ADR-0069](docs/adr/0069-thema-beperkt-tot-leeftijden.md)). A klas is offered, can place and is generated only a thema that holds one of its own leeftijden, and every subthema of a thema holds one of them. A leeftijd cannot be removed while a klas of that leeftijd holds the thema in its jaarplan or a subthema of the thema has it.
  - **`doelsuggesties[]`** — the AI's **FR-4 proposals of a `Minimumdoel` as a themadoel** ([ADR-0052](docs/adr/0052-doelsuggesties-zijn-minimumdoelen.md)), each persisted as `voorgesteld` with `aiMotivatie` and shown best fit first, for a minimumdoel of a mijlpaal that the leeftijden the person chooses meet, by default those of the thema's subthema's (Art. V.1). Deliberately **distinct from the `themadoelen`**: a proposal awaits accept/reject (Art. IV); **accepting it makes its minimumdoel a themadoel**. A rejected or still open one is not proposed again, nor is a current themadoel; a minimumdoel accepted and later unlinked may be. At thema level the AI proposes no leerplandoel: leerplandoelen reach a thema through its subthema's. **Only admins and themabeheer generate and review them** ([Art. VI.1](#article-vi--roles-privacy--security)), because an accepted proposal counts for the dekking of every klas that plans the thema.
...
- **Subdoelvoorstel / Subthemavoorstel** *(age-scoped, on a thema, [ADR-0050](docs/adr/0050-ai-plaatst-leerplandoelen-in-subthemas.md))* — the AI's proposals to place an open leerplandoel of the thema's themadoelen at one leeftijd: in an existing subthema of that leeftijd (a `Subdoelvoorstel` naming it), or in a new one (a `Subthemavoorstel` with naam, `onderzoeksvraag`, `duurWeken`, holding its `Subdoelvoorstel`s). A `Subdoelvoorstel` in an existing subthema also comes from a goal accepted on one of its activiteiten, when that goal is not yet a subdoel there; it names that activiteit ([ADR-0054](docs/adr/0054-ai-zoekt-doelen-bij-een-activiteit.md)). Each has a `status` (voorgesteld / aanvaard / geweigerd / manueel) and an `aiMotivatie`. Accepting one writes an ordinary subdoel, or an ordinary subthema with its subdoelen; until then it counts for nothing (Art. V.1). It goes with its thema, and a `Subdoelvoorstel` with the subthema it names.
- **Activiteitvoorstel** *(personal: of the gebruiker who asked, under one subthema, [ADR-0056](docs/adr/0056-ai-stelt-activiteiten-voor.md))* — an activiteit the AI proposes: naam, `activiteitType?`, `verwachteUitkomsten`, `lengteInLesuren`, the subdoel codes it works on, an optional onderzoeksvraag of the subthema, a `status` (voorgesteld / aanvaard / geweigerd / manueel) and an `aiMotivatie`. Only its asker and an admin see and decide it. Accepting it, changed or not, creates an ordinary **own activiteit** of the asker (ADR-0049) whose goal links are `aanvaard`; until then it counts for nothing (Art. V.1). It goes with its subthema and with its asker. **One the cat brings** ([ADR-0060](docs/adr/0060-activiteitvoorstellen-op-een-aanbod-gat.md)) records that source, its klas, the thema placement it was brought for and a suggested moment; its goals are leerplandoelen in the klas's aanbod-gat, not the subthema's subdoelen; it is addressed to the klas's leerkrachten, and accepting it creates the own activiteit of whoever accepts, planned on that moment.
...
- **Klasfiche** *(klas-scoped, one per klas, [ADR-0057](docs/adr/0057-vervanging-briefing-en-klasfiche.md))* — how the klas runs, in free text per rubric: dagritme, klasafspraken en rituelen, materiaal, praktisch, and a free rubric. Nothing about a child (Art. VI.2). Written by the klas's leerkrachten; the first thing a vervanger's briefing shows. It never counts for dekking.
- **DoelKoppeling** (the link entity, formerly "ThemaDoel") — any link School-content↔Leerplandoel with `status` (voorgesteld / aanvaard / geweigerd / manueel) and `aiMotivatie`. Used for the themadoelen the FR-1 import writes, subdoelen, activity links and algemene fiches alike.

### IX.3 Planning & coverage

- **Schooljaar** — contains multiple klassen; carries its vacations and free days.
...
- **Jaarplan** — klasId; the thema's placed in it, each **from its own first day to its own last day**, with a `vergrendeld` flag per placement ([ADR-0053](docs/adr/0053-themaplaatsing-met-eigen-datums.md)). The teacher picks the first day; the tool proposes the last from the thema's duration in lesweken, and the teacher may change it. **No two placements share a day**, the same thema twice included. **A vacation never lies inside a placement:** a thema that runs over one is stored in parts. When the school changes its vacations, **nothing is moved**: a placement that no longer fits carries a lasting notice and dekking reads *te herzien* until the teacher saves it again. The planning has **no planningsblokken**: a themaperiode and a subthemaperiode are durations, not a grid. **Never hard-assume months.** **A (re)generation may discard only a placement that is `Voorgesteld` and not `vergrendeld`** (FR-7.3's *"ter beslissing"* clause). A placement the teacher has decided on (`Aanvaard` or `Manueel`) survives **without** a lock, so the `vergrendeld` flag only ever changes an outcome for a `Voorgesteld` placement; on a decided one it changes no run outcome, though the "Vast" badge still renders. **Whether the UI offers the lock control there is E4-06's decision and is *not* ratified here.** A teacher rejects a proposal by removing it. Do not paraphrase the discard predicate as "untouched": a change that leaves a placement's days as they were writes nothing, so a placement the teacher did touch stays `Voorgesteld` and is discarded. **A generation proposes thema's with a start week; the calendar sets their days** by the same rules, on free days only, over the whole year ([ADR-0055](docs/adr/0055-ai-jaarplan-met-datums.md)).
  > **Directie's confirmation of the discard rule is outstanding** (question 6 in [`docs/besluiten-gevraagd.md`](docs/besluiten-gevraagd.md)), because FR-7.3 reserved it to them; if directie reverses it, this clause and the teacher-facing strings enumerated under E4-07 change together. **Directie has not been asked about planning with dates** either; it replaces the two-tier block model they settled on 2026-07-14.
...
- **Lesvoorbereiding** *(on one activiteitplaatsing, [ADR-0058](docs/adr/0058-lesvoorbereiding-per-plaatsing.md))* — the AI's preparation of that activiteit on that day in that klas: its goal codes (only the activiteit's decided goals), instap, kern, afsluiting, materiaal, woordenschat, differentiatie, a duur within the plaatsing's hours, a `status` (voorgesteld / aanvaard / geweigerd / manueel) and an `aiMotivatie`. It moves and goes with its plaatsing, and never counts for dekking.
- **Vervanging** *([ADR-0057](docs/adr/0057-vervanging-briefing-en-klasfiche.md))* — a klas, its absent leerkracht, a vervanger, a first day and an optional last day. It gives the vervanger read-only access to the klas while it runs (Art. VI.1). The **briefing** it opens with, and the **terugkeerbriefing** the absent leerkracht finds afterwards, are computed from the planning and never stored.
- **Signaal** *([ADR-0059](docs/adr/0059-de-kat-proactieve-agent.md))* — what the cat noticed about a klas for one recipient: its kind, a key that keeps it unique, and whether it was seen or postponed. The background job derives it without AI and removes it when its reason is gone. It never counts for dekking.
...
- **Activiteitvoorstel** — an activiteit the AI proposes under a subthema to the leerkracht who asked, working on that subthema's subdoelen; once she or an admin accepts it, it is her own activiteit (Art. IX.2). The cat also brings one unasked, on a klas's aanbod-gat.
- **De kat** (the cat) — the AI agent of the app, drawn as a cat: it sleeps in a basket top right, notices without AI what needs attention in a klas, brings proposals, briefings and lesvoorbereidingen, and chats about the tool. It prepares, it never decides (Art. IV.8).
- **Signaal** — something the cat noticed about a klas for one recipient, such as a minimumdoel at risk (Art. IX.3).
- **Aanbod-gat** — for a klas and a discipline, the leerplandoelen of its jaarfase that are in no dekkingsprognose of the klas: no subthema, fiche or activiteit aims at them. Distinct from a **gepland-gat**, a goal in the prognose whose subthema is not yet in the agenda.
- **Vervanging / vervanger** — the recorded absence of a leerkracht from a klas, and the gebruiker who replaces her and meanwhile reads the klas without editing it (Art. VI.1).
- **Briefing / terugkeerbriefing** — the computed page a vervanger sees first when she opens the klas, and the one the absent leerkracht sees when she returns.
- **Klasfiche** — how one klas runs (dagritme, afspraken en rituelen, materiaal, praktisch), written by its leerkrachten for whoever takes the klas over; nothing about a child.
- **Lesvoorbereiding** — the AI's preparation of one planned activiteit: goals, instap, kern, afsluiting, materiaal, woordenschat, differentiatie, duur (Art. IX.3).
...
- **Themaperiode / subthemaperiode** — the days a thema / subthema runs: a thema 4–6 wk, a subthema ±2 wk. Durations, not a grid: a thema placement has its own first and last day (Art. IX.3).
- **Schooldag / lesweek** — a weekday on which the school is open / a Monday-to-Friday week holding at least one schooldag. A thema's duration is counted in lesweken, so a vacation week does not count.
...
- **Jaarplan / themaplaatsing** — the year plan per class / one thema placed in it from its own first day to its own last day (Art. IX.3).
...
- **Gebruiker** — a staff member an admin has added, who logs in over the school's own Entra tenant; the app records which rights they hold (Art. VI.1). Staff only, never a pupil (Art. VI.2).
- **Adminrecht** (admin) — the right to see and edit everything and to maintain gebruikers, klassen, schooljaren and rights. An admin gives it, and several gebruikers may hold it, such as the directeur or an ICT-coördinator. It replaces the former `Beheerder` role ([ADR-0061](docs/adr/0061-de-rol-directie-heet-admin.md)).
- **Themabeheer** — the right, held by a few named leerkrachten or zorgcoördinatoren and given by an admin (FA FR-12.2), to edit thema's with their themadoelen and kernwoordenschat, to run the FR-1 import, to work the thema-opbouw wizard, and to generate and review doelsuggesties (accept or reject them).
...
- **Leerlingzorg** — the right, given by an admin (for example to a zorgcoördinator), to read every ontwikkelingsrapport and nothing else.
...
> **Settled decisions** live in the article they settled: the Op.stap import source (Art. VII.2), planning with the thema's own dates (Art. IX.3), thema scope per level (Art. IX.2), the decreed/autonomous boundary (Art. III), EU hosting and AI data zone (Art. VI.3, [ADR-0016](docs/adr/0016-azure-hosting-eu-residency.md)) and Microsoft Entra ID as identity provider (Art. VI.1, [ADR-0011](docs/adr/0011-authn-authz-rbac-gdpr.md)). The [ratification log](docs/constitutie-log.md) records when each was decided.
