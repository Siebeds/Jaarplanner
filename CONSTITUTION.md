...
2. Every AI output (goal match, generated plan) must be **reviewable and accept/reject/adjustable**, and its **status must be persisted** (`voorgesteld` / `aanvaard` / `geweigerd` / `manueel`).
   A thema's **doelsuggestie**, the proposal of a minimumdoel as themadoel, is accepted or rejected and not adjusted: whoever wants another minimumdoel rejects the proposal and links that one by hand ([ADR-0049](docs/adr/0049-doelsuggesties-zijn-minimumdoelen.md)).
...
1. **Dekking is computed, never stored**, per klas, in **two steps that are both shown**: the **dekkingsprognose**, what the school's thema's and subthema's aim at, and the **dekking**, what the klas's agenda holds. Only a decided link (status `aanvaard` or `manueel`) counts ([ADR-0047](docs/adr/0047-dekkingsprognose-en-dekking.md)).
   - A `Minimumdoel` is in the *dekkingsprognose* when it is a themadoel of at least one thema, and *gedekt* when such a thema is placed (`aanvaard` or `manueel`) in the klas's plan. **It counts only through a thema it is itself linked to**, never through a concorded leerplandoel. An accepted doelsuggestie counts only through the themadoel its acceptance made ([ADR-0049](docs/adr/0049-doelsuggesties-zijn-minimumdoelen.md)).
   - A `Leerplandoel` of the klas's jaar/fase is in the *dekkingsprognose* when it is a subdoel of, or linked to an activiteit of, a subthema at the klas's leeftijd. It is *gedekt* when that **subthema is placed in the klas's agenda** (placing only the thema above it is not enough).
