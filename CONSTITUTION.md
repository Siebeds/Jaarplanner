...
4. The AI is **grounded only on the school's own data** (its thema's/activiteiten) and the loaded Op.stap goals — **never** external or unknown sources. A rewrite is grounded on the teacher's own text only, with the rapportdoel's title and the gradatie's label, and the names of the klas's children are replaced before the call (Art. VI.7).
5. AI calls **always request structured JSON** (goal codes + one-line motivation; or planningsblok→thema's for plan generation; or one rewritten text carrying exactly the placeholders it was sent) and the response is **validated before use**.
...
7. **The ontwikkelingsrapport (K3 only)** *(owner rulings of 2026-09-14, [ADR-0035](docs/adr/0035-ontwikkelingsrapport-derde-kleuter.md) §1; entities in Art. IX.4)* is the one place pupil data lives.
   - **What is stored:** a leerling's voornaam and achternaam, typed by hand (R14, R15), and per evaluatiemoment a gradatie and a text per rapportdoel, an algemeen besluit and at most one kindtekening (R8 to R10). Nothing else about a child. Only a klas that grants K3 has leerlingen, through the one klas→leeftijden mapping of VI.1 (default D9).
   - **Who:**
     - the klas's leerkrachten fill it in during its schooljaar, and afterwards only read and download it (R16, R26);
     - directie does everything;
     - **Leerlingzorg** reads it (R18);
     - every K3 leerkracht and directie edit the one K3 set of rapportdoelen and the one scale (R4 to R6).

     Nobody else reads a report, whatever I9 grants for plans (R17).
   - **AI:** a rewrite of a text or of the besluit (R21, R22) is sent with the names of every child of the klas replaced, and they are restored after the call. The filter is best-effort, and the teacher is told so (R25). Pseudonymised text is still personal data.
   ...
   - **Logs** carry no pupil content, and **the repository** never holds a real child's name (defaults, ADR-0035 §3.8).
   - **Deletion:** kept until directie wipes a schooljaar (R19), and the beheerpagina shows which schooljaren still hold pupil data (R28). A leerling can be deleted with their reports at any time (default D8).
   - **Export:** PDF and Word, made on demand and never stored (R13). Per rapportdoel the parent sees the title, the star with its label, and the text (R11).
   - **It never counts for dekking** (Art. V is unchanged).

   **This clause is ratified only as far as the rulings (the R-items of ADR-0035) it cites.** Everything else in ADR-0035 §2 and §3 is the recording session's design, D1 to D16 included: for example which subdoelen may be bundled, the signed rewrite proposals, the licence rule and where images are stored. It is a default, followed by the build until the owner changes it, and it is **not** ratified here. *Directie has not confirmed the rulings* (question 15).
...
| 2026-09-14 | Siebe De Saedeleir (projecteigenaar) | Brought the **ontwikkelingsrapport for the derde kleuter** into scope: the first pupil data the app holds. Recorded as [ADR-0035](docs/adr/0035-ontwikkelingsrapport-derde-kleuter.md), whose §1 quotes the 29 rulings (R1 to R29) with the options offered. *Amended:* **Art. I.1** (item 9). **Art. I.2** (pupil-level reporting and grading, each with the ontwikkelingsrapport excepted and bounded; the other three non-goals unchanged). **Art. IV.1 to IV.5** for the AI rewrite, a third kind of AI output: the proposal is not stored, every decision is, and a rejection is stored without its text (R23); no motivation (R24). **Art. VI.1** (a fifth right, Leerlingzorg, R18; the report outside I9, R17; its rows in the ADR-0030 matrix). **Art. VI.2** and a new **Art. VI.7**. **Art. VI.6**: the owner's waiver of the order, so real pupil data may enter before the register entry, before the parents are informed and before a DPIA (R20, R27), chosen twice against the recommendation with the AVG articles stated. A new **Art. IX.4**. Nine **Art. XII** entries. *Amended in step (Art. XI.1):* the functional analysis (§2 *Buiten scope*, §3.1, a new FR-13, NFR-6, §10, A.11); CLAUDE.md; the ADR-0030 matrix; the status of ADR-0011 and ADR-0016 and the ADR index; E7-06 and the E8 note; question 15 in `docs/besluiten-gevraagd.md`. Also, on the owner's ruling that the amendment may not block what he asked for (R29), the antagonist, implementer and jaarplan-build instructions, which now treat pupil data outside the report as a violation and check it inside the report against VI.7. *Why:* the K3 teachers write this report by hand three times a year, and the owner wants it where their subdoelen already are. *Scope of the ratification:* the R-items only. Everything else in ADR-0035 §2 and §3, D1 to D16 included, is the recording session's design and a default. *How it was ratified:* in session, through five rounds of multiple-choice questions. The fifth came after the antagonist's first audit (3 MAJOR, 8 MINOR), which found session design presented as ratified. *Directie's confirmation is outstanding* (question 15), and as verwerkingsverantwoordelijke the school carries what R20 and R27 waive. |
...
- **Ordering & graadklassen**: follow Op.stap jaar/fase ordering (JK–L6, mijlpalen K3/L4/L6); how to handle graadklassen / menggroepen? *Partly answered for rights, provisionally, by the owner's rulings of 2026-09-13 (Art. VI.1; [ADR-0030](docs/adr/0030-rollen-en-rechten-in-de-app.md) R22): a graadklas grants the leeftijd rights of its one jaarfase, and the klas→leeftijden mapping for rights lives in one place. Directie has not been asked, and the question stays theirs.*
