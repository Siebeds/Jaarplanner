...
## Status
**Phase 3 — in progress.** **Milestones M0 and M2 reached; M1 and M3 are not.** E0 (foundation & scaffolding) and E2 (AI-matching thema ↔ doel) are complete. **E1 is not complete** and **M1 is *not* reached**, for two independent reasons: E1-03/E1-04 are reopened and blocked on **E1-12** (decreed-minimumdoelen import), which needs the source file from directie — until then no `Minimumdoel` row can exist and minimumdoel-level coverage, the level the onderwijsinspectie tests, returns nothing; and **E1 has no UI whatsoever** (`frontend/src/features/` holds only `matching`), so every E1 FR whose wording names a user action is unsatisfied despite the `[x]` on its server-side story — tracked as E1-13 and E1-14. Within **E3 — Jaarplan-generatie & kalender** (M3), E3-10 (wireframe, approved) and E3-05 (planningsblok model) are done; next is **E3-01** (jaarplan generation service, which also owns the `Jaarplan` entity and Schooljaar↔Klas containment).
...
- **After any significant change, invoke the `antagonist` subagent** (`.claude/agents/antagonist.md`) to audit the change against `CONSTITUTION.md`. A *significant change* = new/modified source files, data-model or migration changes, Excel-import or coverage logic, AI prompts/orchestration, permissions, or any scope-touching edit. Address or explicitly waive its findings before considering the task done. Trivial doc tweaks are exempt.
  ...
  >
  > *Registration:* all three resolve as normal subagent types (`Agent(subagent_type: "antagonist")`). They previously did not, because the harness resolves `.claude/agents/` against the session cwd while the repo sat one level below it; the repo was moved up on 2026-07-28 (`b2257f7`) and that is fixed — verified by an antagonist audit running as a registered agent the same day. Do **not** substitute a self-audit — the independent pass has repeatedly caught defects a self-review missed, including in the fixes written to address its own earlier findings.
