...
- **A thema placement has its own first and last day** ([ADR-0049](docs/adr/0049-themaplaatsing-met-eigen-datums.md)): no two share a day, a vacation splits one into parts, and nothing moves when the vacations change. The calendar rules (schooldag, lesweek, proposed end, split) live in `Themakalender`. There are no planningsblokken in the planning; never assume months.
