...
## Status
**Phase 3 — in progress.** **M0 and M2 are reached.** **M2 was restored 2026-07-30** by E2-08, which gave FR-4.1 the invocation surface it never had: a teacher generates doelsuggesties from `/themas` and reviews them, verified in a browser against a real API and PostgreSQL. *(Since the owner's rulings ratified on 2026-09-14, only directie and themabeheer may do this (Art. VI.1); E6-02 gates that control.)* **E2 is complete** apart from **E2-09** (a missing AI config returns 500 where 503 belongs — filed, not blocking).

**E1 is not complete** and **M1 is *not* reached** (its gaps are kept in [`backlog/README.md`](backlog/README.md)). The curriculum half has its input since 2026-09-13: **E1-21** imports the G goals from KOV's Op.stap API ([ADR-0032](docs/adr/0032-opstap-api-als-importbron.md)), and 4,983 of them concord to 992 of the 998 minimumdoelen that **E1-12** imports from the same API; with it **E1-12, E1-03 and E1-04 closed on the input** (owner ruling 2026-09-13). What is still missing is the computation: minimumdoel-level coverage, the level the onderwijsinspectie tests, is **E5-04**, which owns that clause since the same ruling and which these stories unblock. After the first API import of the leerplandoelen the Excel Op.stap route refuses every file (Art. VII.2 as amended 2026-09-13). **E1-16 closed 2026-07-31** and gave E1 its first screen (the Doelen-register behind the nav item E0-10 shipped), so "E1 has no UI at all" is no longer true; both teacher-facing halves now exist: **E1-13** landed the import flow and **E1-14 closed 2026-08-04** with the thema-/activiteitenbeheer screens, in two landings and after four antagonist rondes. *(Rewritten 2026-09-13: this paragraph said M1 was blocked on E1-12 and E1-21, and until 2026-09-11 that E1-12 needed the decreed file from directie; both became false as the API route landed.)*
...
- **No work without a ticket or a story** *(owner, 2026-09-13, [ADR-0033](docs/adr/0033-ticketbacklog-en-kanbanbord.md))*. New work enters through `backlog/functionele-backlog/` (FB, logged by the functional architect on `main`) and `backlog/technische-backlog/` (TB), in the one format described in [`backlog/TICKETS.md`](backlog/TICKETS.md). **Before changing any file for work that is neither an `E<n>-<nn>` story nor an existing ticket, create a TB ticket first** (skill **`ticket-aanmaken`**), then work it with **`ticket-uitvoeren`**: a ticket's status and Werklog change only through `node tools/backlog-board/tickets.mjs`, the final status (`te-testen` for FB, `klaar` for TB) goes in the branch's last commit, and every gate below applies unchanged. Only the owner changes an FB ticket's status by hand, on his own PC, where the board runs and the sessions work: he moves it to `klaar-voor-bouw` and, once the functional architect has tested it, records the result with **`ticket-testen`**. The functional architect creates tickets, always as `nieuw`, and may sharpen their text while they are `nieuw` (owner rulings 2026-09-13). Epic stories keep the `jaarplan-build` flow and never get a ticket; a new story is filed only as a follow-up inside an epic that is still open, for work that epic needs to finish, and all other new work is a ticket. *Tickets are written in Dutch* (owner ruling, Art. II.6), the one exception to the English default for backlog text; their commit messages and the CLI's command and option names stay English. A CLI write refuses when a newer copy elsewhere says a different status, holder or block, and names the git command that fixes it; after the merge a ticket is written on `main`, and a blocked ticket is neither picked up nor given back (the owner's `release` of a stopped session is the one exception: the block stays on that copy until he answers it). Never work around a refusal. Exempt: questions, explanations, read-only investigation, edits to ticket files themselves, and bookkeeping (the coordination state under `.claude/coordination/`, epic checkboxes, the progress table and the technical lead's backlog corrections).
...
- **AI flow:** frontend requests a match or generated plan → `Application` builds a prompt from the relevant doelen + thema's/activiteiten → calls Azure AI Foundry → parses a **structured JSON** response → returns suggestions with a short motivation and `status = voorgesteld`. Whoever holds the right accepts/rejects in the UI: directie or themabeheer for a thema's doelsuggesties, a leerkracht of the klas for plan placements (Art. VI.1).
- **Goals data flow:** KOV's Op.stap API → `Infrastructure` import (backend only; preview, then apply) → `Leerplandoel`/`Minimumdoel` tables ([ADR-0032](docs/adr/0032-opstap-api-als-importbron.md); the per-discipline Excel import still exists but is no longer the source). School data (thema's/activiteiten) arrives via a separate Excel upload (FR-1).
...
- **Minimumdoel** — ref, leeftijd (K-/4-/6-), nr, omschrijving, and the decree's own ordering `leergebied › rubriek › subrubriek?` with its `soort` (from KOV's `path` and `type`; Art. IX.1 as amended 2026-09-14, TB-010). All four are nullable: a row imported before TB-010 has none until the next minimumdoelen import, a two-level path has no subrubriek, and a path or type the mapping cannot read leaves them empty. The decreed eindterm; concorded to leerplandoelen. The minimumdoelen register is browsed in the decree's ordering; the decree's leergebied is neither a discipline nor the teachers' leergebied / Wereldoriëntatie (Art. XII).
- **Thema** — id, naam, subthema's[], activiteiten[]. Edited by directie and themabeheer only (Art. IX.2, VI.1).
...
- **Dekking** is computed, not stored: a leerplandoel is *gedekt* when linked (status aanvaard/manueel) to a thema placed in the plan, **or** to an algemene fiche of the klas that is planned in its agenda (owner ruling 2026-09-11, [ADR-0029](docs/adr/0029-algemene-fiches.md)); a minimumdoel is *gedekt* when ≥1 concorded leerplandoel is gedekt.
...
- **Gebruiker and rights** — only a gebruiker directie added logs in, and the app (not Entra) records its rights ([`CONSTITUTION.md` Art. VI.1](CONSTITUTION.md#article-vi--roles-privacy--security), ratified 2026-09-14).
  - **Directie** sees and edits everything and maintains gebruikers, klassen, schooljaren and rights. It may give the directie right to someone else, e.g. an ICT-coördinator.
  - **Themabeheer** edits thema's, runs the FR-1 import (goal links included; only directie may switch on its option to delete human decisions) and the thema-opbouw wizard, and generates and reviews doelsuggesties. Through the wizard's own actions it creates subthema's, subdoelen and activiteiten only for a thema it builds from scratch.
  - A **hoofdleerkracht**, a gebruiker appointed per (schooljaar, jaarfase), several allowed, creates, edits and deletes that jaarfase's subthema's and subdoelen, links goals to its shared activiteiten by hand, and deletes any of them.
...
- ~~Whether a leerplandoel/thema is shared school-wide or per class.~~ — **resolved (2026-06-29, narrowed 2026-08-30):** thema's are school-wide, and subthema's and their content are per leeftijd ([`CONSTITUTION.md` Art. IX.2](CONSTITUTION.md#article-ix--core-data-model-functional)). Who may edit which is Art. VI.1 (ratified 2026-09-14).
