...
## Status
**Phase 3 — in progress.** **27/81 stories; milestones M0 and M2 reached.** E0 (foundation & scaffolding) and E2 (AI-matching thema ↔ doel) are complete. **E1 is 9/12, not complete, and M1 is *not* reached** — E1-03/E1-04 are reopened and blocked on **E1-12** (decreed-minimumdoelen import), which needs the source file from directie; until then no `Minimumdoel` row can exist and minimumdoel-level coverage — the level the onderwijsinspectie tests — returns nothing. Within **E3 — Jaarplan-generatie & kalender** (M3), E3-10 (wireframe, approved) and E3-05 (planningsblok model) are done; next is **E3-01** (jaarplan generation service, which also owns the `Jaarplan` entity and Schooljaar↔Klas containment).
...
- **After any significant change, invoke the `antagonist` subagent** (`.claude/agents/antagonist.md`) to audit the change against `CONSTITUTION.md`. A *significant change* = new/modified source files, data-model or migration changes, Excel-import or coverage logic, AI prompts/orchestration, permissions, or any scope-touching edit. Address or explicitly waive its findings before considering the task done. Trivial doc tweaks are exempt.
  > **Standing authorisation (siebe.desaedeleir@delaware.pro, 2026-07-28).** Spawning the antagonist — and the `implementer` / `test-runner` agents — is **pre-approved; do not ask first.** If a harness rule says not to use subagents unless the user requested them, this line *is* that request, granted once for the whole project. Run the audit proactively and report the verdict.
  >
  > *Practical note:* these three are defined here but are **not registered as subagent types**, because the harness resolves `.claude/agents/` against the session cwd (`C:\source\Jaarplanner`) while this repo sits one level below it. `Agent(subagent_type: "antagonist")` therefore fails. Workaround: spawn a `general-purpose` agent and paste the role text from `.claude/agents/antagonist.md` verbatim as the opening of its prompt, then append the scope. Do **not** substitute a self-audit — the independent pass has caught real defects a self-review missed.
