...
**E0 is a complete epic again**: **E0-10** — the app shell no story owned (routing, navigatie, klas/schooljaar-keuze, plus a UI redesign) — **closed 2026-07-30** after the browser pass and the antagonist audit. Within **E3** (M3), E3-01, E3-02, E3-05, E3-10 and **E3-04** are done; **E3-06** is `[~]` awaiting the directie/teacher review that closes it, which the owner postponed to near the end of the build; E3-03 is blocked on E5. **E3-07** (drag-and-drop) was closed 2026-07-30 and is **reopened `[~]` since 2026-08-03 by owner ruling**: E4-06's gates found, in E3-07's own copy and its own picker, a stale *rejected* card that both invites the teacher to pick a period and says the thema cannot be moved, with no picker rendered. One card state, not a rebuild; everything else about the story stands. **E3-04 closed 2026-07-31** once the browser pass finally ran (headless Chrome from Bash, after both gate agents died on an org spend limit): the generation settings are kept per klas × schooljaar, which is what the owner's 2026-07-30 ruling required and what E4's regeneration inherits.

**E4 has started.** **E4-06** (vergrendelde plaatsingen survive (her)generatie) closed 2026-08-03 and is **merged and pushed to `main`** (`61457bc`): the lock had a server, an endpoint and a badge and no switch, the fourth instance of the reachable-vs-tested gap after E2-08, E1-15 and E0-10.
...
- **Any UI work starts with the `frontend-design` skill.** *(Standing instruction from the project owner, 2026-07-29.)* Before building a new screen or changing how an existing one looks, invoke the **`frontend-design`** skill and design deliberately against this application's own principles — not the generic defaults. That means:
  ...
  - **A conditional sentence may assert only what its own render condition guarantees** *(the E5-03 rule, 2026-08-06)*. Ask what the branch you are writing inside actually proves, and delete every clause that reaches past it. Four false sentences on one story all had this shape: *"Kies bij Doelsoort een andere soort"* asserted a **control** another branch owned; *"tellen mee in dit cijfer"* presupposed a **figure** another branch owned; and two versions of one empty-state line asserted things owned by `DekkingService`. It catches code comments too, which is why it is one rule and not two: *"a filter hides rows and leaves the figure alone"* and *"a test pins that equality"* each described code elsewhere in the tree. **The corollary is the half that saves you: when the honest explanation is forbidden, say less, never say something else.** Both false empty-state versions filled an explanatory slot that a *ruling* had emptied, and that situation recurs by design here (the export FR-11 and the minimumdoel level E5-04 both have facts the tool may not yet state). This binds **conditional** copy; unconditional page-level text is judged against the code as a whole. *A `t(key)` assertion cannot catch any of it, because the expectation moves with the catalogue: the guards that can read the **value** and live in `frontend/src/i18n/catalogus.test.ts`.*
