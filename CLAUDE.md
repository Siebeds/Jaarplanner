...
- **AI flow:** the backend builds a prompt from the school's own doelen and thema's, calls the configured provider, validates a **structured JSON** response, and returns suggestions with a motivation and `status = voorgesteld`. The client sits behind an interface and is faked in tests. Two exceptions to grounding on school data alone (Art. IV.4): a woordweb's words (ADR-0043), and the name and onderzoeksvraag of a thema or subthema the AI proposes (ADR-0050); the goals it places are always loaded Op.stap goals.
