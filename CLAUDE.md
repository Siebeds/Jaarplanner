...
- **AI flow:** the backend builds a prompt from the school's own doelen and thema's, calls the configured provider, validates a **structured JSON** response, and returns suggestions with a motivation and `status = voorgesteld`. The client sits behind an interface and is faked in tests. Three exceptions to grounding on school data alone (Art. IV.4): a woordweb's words (ADR-0043), the name and onderzoeksvraag of a thema or subthema the AI proposes (ADR-0050), and the content of an activiteit it proposes (ADR-0054); the goals it links are always loaded Op.stap goals.
...
- **A (re)generation discards only a placement that is `Voorgesteld` and not `vergrendeld`.** The generation is switched off until it is reworked for dates. Generatieparameters are kept per (klas, schooljaar); a vast moment blocks nothing.
...
- An activiteitvoorstel (ADR-0054) is personal: the AI proposes activiteiten on a subthema's subdoelen to whoever may create an own activiteit there, only she and directie see and decide them, and an accepted one becomes her own activiteit.
