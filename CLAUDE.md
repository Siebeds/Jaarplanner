...
**E0 reopened** for **E0-10**, the app shell no story owned (routing, navigatie, klas/schooljaar-keuze, plus a UI redesign). It is merged to `main` but still `[~]`: its gates have not run. Within **E3** (M3), E3-01, E3-02, E3-05 and E3-10 are done; **E3-06** is `[~]` awaiting the directie/teacher review that closes it; E3-03 is blocked on E5. **E3-07** (drag-and-drop) is `[~]` — built 2026-07-30 and verified in a browser, awaiting its antagonist audit; the owner **waived** the teacher review for it, so it no longer waits on E3-06. **E3-04** is being built in a parallel worktree.
