...
- **AI flow:** the backend builds a prompt from the school's own doelen and thema's, calls the configured provider, validates a **structured JSON** response, and returns suggestions with a motivation and `status = voorgesteld`. The client sits behind an interface and is faked in tests. The AI is sent only school data and loaded goals; it may make up **content** from its own knowledge (woordweb words, thema and subthema names, activiteiten, lesvoorbereidingen, hoekverrijkingen), but every goal it names or links is a loaded Op.stap goal it was sent (Art. IV.4, ADR-0058).
- **The cat** ([ADR-0059](docs/adr/0059-de-kat-proactieve-agent.md)) is the one AI that acts unasked: a background job in the API process (one instance per tick) derives signals per klas **without AI**, and calls the AI only for the lesvoorbereidingen of a vervanging and for activiteitvoorstellen on an aanbod-gat. It prepares, never decides. Its chat stores nothing.
...
- `Thema`, `Themadoel` and kernwoordenschat are school-wide, but a thema holds the **leeftijden** it is meant for (all by default): a klas is offered only the thema's of its own leeftijd, and a thema's subthema's stay inside them ([ADR-0069](docs/adr/0069-thema-beperkt-tot-leeftijden.md)); `Subthema`, `Subdoel` and `Activiteit` are per **leeftijd** (jaarfase), not per klas; `AlgemeneFiche` is per klas. A klas has a required `jaarfase`.
...
- Rights, checked server-side: admin sees and edits everything, and several gebruikers may hold it ([ADR-0061](docs/adr/0061-de-rol-directie-heet-admin.md)); themabeheer edits thema's, runs the FR-1 import and the wizard, and with admin alone reviews doelsuggesties; a hoofdleerkracht per (schooljaar, jaarfase) owns that jaarfase's subthema's, subdoelen and goal links; a klastoewijzing gives a klas's planning; a leerkracht and a hoofdleerkracht read the planning of the klassen of their own jaarfase, themabeheer every klas, a vervanger the klas of her running vervanging, anyone else none ([ADR-0040](docs/adr/0040-klassen-inkijken-per-jaarfase.md), ADR-0057); Leerlingzorg reads every ontwikkelingsrapport.
...
- A `Woordweb` (the brainstorm of step 3, [ADR-0043](docs/adr/0043-eigen-woordweb-per-subthema.md)) is personal: one per (gebruiker, subthema), read by everyone, edited by its owner and an admin. Its AI words come from the model's own language knowledge (Art. IV.4), and it never counts for dekking.
