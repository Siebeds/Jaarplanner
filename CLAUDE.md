...
## Status
**Phase 3 — in progress.** **Only M0 is reached.** E0 (foundation & scaffolding) is complete. **M2 is withdrawn** (2026-07-28): E2's accept/reject UI works, but `MatchThemaAsync` is called from nothing but its own unit tests, so no suggestion can ever be generated in a running app — a teacher does not get suggestions, which is what M2 asserts. Tracked as E2-08. **E1 is not complete** and **M1 is *not* reached**, for two independent reasons: E1-03/E1-04 are reopened and blocked on **E1-12** (decreed-minimumdoelen import), which needs the source file from directie — until then no `Minimumdoel` row can exist and minimumdoel-level coverage, the level the onderwijsinspectie tests, returns nothing; and **E1 has no UI whatsoever** (`frontend/src/features/` holds only `matching`), so every E1 FR whose wording names a user action is unsatisfied despite the `[x]` on its server-side story — tracked as E1-13 and E1-14. Within **E3 — Jaarplan-generatie & kalender** (M3), E3-10 (wireframe, approved), E3-05 (planningsblok model) and **E3-01** (jaarplan generation service, which also owns the `Jaarplan` entity and Schooljaar↔Klas containment) are done; next is **E3-02** (spreading heuristics) or **E3-06** (calendar view).
